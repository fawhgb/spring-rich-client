 ...
 *
 * <p>
 * Only usable on Java 5. Use an appropriate {@link JdkVersion} check before
...
public abstract class GenericCollectionTypeResolver {
	...
	private static Class getGenericParameterType(MethodParameter methodParam, Class source, int typeIndex) {
		return extractType(methodParam, methodParam.getGenericParameterType(), source, typeIndex,
	...
	private static Class extractType(MethodParameter methodParam, Type type, Class source, int typeIndex,
		...
		if (type instanceof TypeVariable && methodParam != null
				&& GenericTypeResolver.getTypeVariableMap(source) != null) {
			Type mappedType = GenericTypeResolver.getTypeVariableMap(source).get(type);
			if (mappedType != null) {
	...
	private static Class extractTypeFromParameterizedType(MethodParameter methodParam, ParameterizedType ptype,
		...
		if (paramType instanceof TypeVariable && methodParam != null
				&& GenericTypeResolver.getTypeVariableMap(source) != null) {
			Type mappedType = GenericTypeResolver.getTypeVariableMap(source).get(paramType);
			if (mappedType != null) {
	...
	/**
	 * Determine whether the given class is a potential candidate that defines
	 * generic collection or map types.
	 * 
	 * @param clazz the class to check
