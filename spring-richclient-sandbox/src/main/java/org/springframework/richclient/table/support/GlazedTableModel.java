 ...
 * {@link AdvancedTableFormat} is provided for use. It allows for the
 * specification of an object prototype (for determining column classes) and the
 * ability to specify comparators per column for sorting support.
 * <p>
 * This model can be given an Id, which is used in obtaining the text of the
 * column headers.
 * <p>
 * Column header text is generated from the column property names in the method
 * {@link createColumnNames}. Using the field face source configured, or the
 * default application field face source if none was configured.
 *
...
public class GlazedTableModel extends EventTableModel {
	...
	/**
	 * Constructor using the provided row data and column property names. The model
	 * Id will be set from the class name of the given <code>beanClass</code>.
	 * 
	 * @param beanClass
	 * @param rows
	 ...
	 * @param modelId             Id for this model, used to create column header
	 *                            message keys
	 */
	 ...
	 * column property name to generate a series of message keys. Resolve those keys
	...
	/**
	 * Construct the table format to use for this table model. This base
	 * implementation returns an instance of {@link DefaultTableFormat}.
	 * 
	 * @return
	 */
	...
	/**
	 * This inner class is the default TableFormat constructed. In order to extend
	 * this class you will also need to override
	 * {@link GlazedTableModel#createTableFormat()} to instantiate an instance of
	 * your derived table format.
	 */
	 ...
	 * <p>
	 * If a prototype value is provided (see {@link #setPrototypeValue(Object)},
	 * then the default implementation of getColumnClass will inspect the prototype
	 * object to determine the Class of the object in that column (by looking at the
	 * type of the property in that column). If no prototype is provided, then
	 * getColumnClass will inspect the current table data in order to determine the
	 * class of object in that column. If there are no non-null values in the
	 * column, then getColumnClass will return Object.class, which is not very
	 * usable. In that case, you should probably override
	...
	protected class DefaultAdvancedTableFormat implements AdvancedTableFormat {
		 ...
		 * object has been specified, then the class will be obtained using
		 * introspection using the property name associated with the specified column.
		 * If no prorotype has been specified, then the current objects in the table
		 * will be inspected to determine the class of values in that column. If no
		 * non-null column value is available, then <code>Object.class</code> is
		 * returned.
		 * 
		...
		/**
		 * Set the prototype value from which to determine column classes. If a
		 * prototype value is not provided, then the default implementation of
		 * getColumnClass will return Object.class, which is not very usable. If you
		 * don't provide a prototype, you should probably override
		 * {@link #getColumnClass(int)}.
		 */
