...
public class JdbcSettingsFactory implements SettingsFactory, InitializingBean {
	...
	public Settings createSettings(String key) throws SettingsException {
		try {
			...
			Map result = template.queryForMap("SELECT * FROM SETTINGS WHERE KEY=? AND USER=?",
					new Object[] { key, userNameProvider.getUser() });

			JdbcSettings settings = new JdbcSettings(dataSource, userNameProvider.getUser(), (Integer) result.get("ID"),
					key);
			settings.load();
			return settings;
