 ...
 *
 * <p>
 * Mainly intended for usage within the framework, determining the target type
 * of values to be added to a collection or map (to be able to attempt type
 * conversion if appropriate).
 *
 * <p>
 * Only usable on Java 5. Use an appropriate JdkVersion check before calling
 * this class, if a fallback for JDK 1.3/1.4 is desirable.
 *
...
public abstract class GenericCollectionTypeResolver extends org.springframework.core.GenericCollectionTypeResolver {
	...
	/**
	 * Determine the generic element type of the given Collection class (if it
	 * declares one through a generic superclass or generic interface).
	 *
	...
	/**
	 * Determine the generic key type of the given Map class (if it declares one
	 * through a generic superclass or generic interface).
	 *
	...
	/**
	 * Determine the generic value type of the given Map class (if it declares one
	 * through a generic superclass or generic interface).
	 *
	 * @param mapClass the map class to introspect
	 * @return the generic type, or <code>null</code> if none
	 ...
	 *
	 * @param collectionField the collection field to introspect
	 * @param nestingLevel    the nesting level of the target type (typically 1;
	 *                        e.g. in case of a List of Lists, 1 would indicate the
	 *                        nested List, whereas 2 would indicate the element of
	 *                        the nested List)
	 * @return the generic type, or <code>null</code> if none
	 */
	 ...
	 *                     List, whereas 2 would indicate the element of the nested
	 *                     List)
	 * @return the generic type, or <code>null</code> if none
	 */
	 ...
	 * Determine the generic element type of the given Collection return type.
	 * <p>
	 * If the specified nesting level is higher than 1, the element type of a nested
	 * Collection/Map will be analyzed.
	 *
	 * @param method       the method to check the return type for
	 * @param nestingLevel the nesting level of the target type (typically 1; e.g.
	 *                     in case of a List of Lists, 1 would indicate the nested
	 ...
	 *
	 * @param methodParam the method parameter specification
	 * @param typeIndex   the index of the type (e.g. 0 for Collections, 0 for Map
	 *                    keys, 1 for Map values)
	 * @return the generic type, or <code>null</code> if none
	 */
	...
	/**
	 * Returns the source class for the specified type. This is either
	 * <code>Collection.class</code> or <code>Map.class</code>. If none of both
	 * matches the baseclass of the specified type is returned.
	 *
	 ...
	 * @param source       the source class/interface defining the generic parameter
	 *                     types
	 * @param typeIndex    the index of the type (e.g. 0 for Collections, 0 for Map
	 *                     keys, 1 for Map values)
	...
	private static Type extractType(MethodParameter methodParameter, Type type, int typeIndex, int nestingLevel) {
		for (int i = 0; i <= nestingLevel; i++) {
			...
			int indexToUse = (currentTypeIndex != null ? currentTypeIndex.intValue()
					: i == nestingLevel ? typeIndex : getValueTypeIndex(type));
	...
	public static Type getActualType(ParameterizedType type, Class superType, int typeIndex) {
		...
		if (genericSuperclass instanceof ParameterizedType) {
			...
			if (actualType instanceof TypeVariable) {
				actualType = getActualType((TypeVariable) actualType, rawType.getTypeParameters(),
	...
	private static Type getActualType(TypeVariable typeVariable, TypeVariable[] typeVariables,
