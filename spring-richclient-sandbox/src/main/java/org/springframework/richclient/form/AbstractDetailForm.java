...
/**
 * This is an abstract base implementation of the detail side of a Master/Detail
 * form pair. Derived types need only implement
 * {@link AbstractForm#createFormControl()}.
 * <p>
 * The various form commands can be secured by specifying security controller
 * id's for the command. Use the
 * {@link AbstractForm#getCommitSecurityControllerId()} and
...
public abstract class AbstractDetailForm extends AbstractForm implements PropertyChangePublisher {
	...
	/**
	 * Construct a detail form using the provided parent form model (we will
	 * construct our own form model as a child of the parent model). The provided
	 * masterList will be installed as the set of editable form objects.
	 * 
	...
	/**
	 * Get the value holder containing the editing index. This allows triggers to
	 * monitor for changes in the index of the object we are editing.
	 * 
	 * @return
	 */
	...
	/**
	 * Set the form for "create new object" mode. This will set controls as needed
	 * for this edit mode.
	 */
	...
	/**
	 * Get the current edit state: one of {@link #STATE_CLEAR},
	 * {@link #STATE_CREATE}, or {@link #STATE_EDIT}.
	 * 
	...
	/**
	 * Commit this forms data back to the master table. Let our super class do all
	 * the work and then just inform our master table that the value has changed.
	 */
	@Override
	...
	/**
	 * Override to return null for the new object security controller id. We do this
	 * because this command is not used directly, so it shouldn't be controlled. The
	 * {@link AbstractMasterForm} is responsible for the real (invocable) instance
	 * of this command.
	 * 
	 * @return null
	 */
	...
	/**
	 * Create the cancel command. This will cancel the "create new" operation and
	 * reset the form.
	 * 
	...
	protected JComponent createButtonBar() {
		...
		formCommandGroup = CommandGroup.createCommandGroup(null,
				new AbstractCommand[] { cancelCommand, revertCommand, commitCommand });
