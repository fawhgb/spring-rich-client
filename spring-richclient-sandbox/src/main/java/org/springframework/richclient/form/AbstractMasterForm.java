...
/**
 * Abstract base for the Master form of a Master/Detail pair. Derived types must
 ...
 * {@link AbstractForm#createControl()} <strong>MUST</strong> call
 * {@link #configure()} prior to its work in order to have the detail form
 * properly prepared.
 *
...
public abstract class AbstractMasterForm extends AbstractForm {
	...
	/**
	 * Construct a new AbstractMasterForm using the given parent form model and
	 * property path. The form model for this class will be constructed by getting
	 * the value model of the specified property from the parent form model and
	 * constructing a DeepCopyBufferedCollectionValueModel on top of it.
	 *
	 * @param parentFormModel Parent form model to access for this form's data
	 * @param property        containing this forms data (must be a collection or an
	 *                        array)
	...
	protected AbstractMasterForm(HierarchicalFormModel parentFormModel, String property, String formId,
		...
		ValidatingFormModel formModel = FormModelHelper.createChildPageFormModel(parentFormModel, formId, collectionVM);
	...
	/**
	 * Get the value model representing the collection we are managing. This value
	 * model can be used to register vlue change listeners and to update the
	 * collection contents.
	 * <p>
	 * You must use this method to get the value model since calling getValueModel
	 * on the parent form model will not get you what you want.
	 *
	...
	/**
	 * Determine the type of the collection holding the detail items. This will be
	 * used to create the value model for the collection.
	 * <p>
	 ...
	 * error regarding lazy instantiation since the new PersistentList instance
	 * would not have been properly initialized by Hibernate.
	 *
	 ...
	 * @param valueHolder     BufferedValueModel holding an object of the type
	 *                        configured for this master form.
	 * @param masterList      The ObservableList of data to from the master form
	 *                        (this will constitute the editable object list for the
	 *                        detail form).
	 */
	...
	/**
	 * Install an EventList for use as our master list data. The event list must
	 * have been created on top of the event list obtained from
	 * {@link #getRootEventList()} so that all changes are propery proxied onto the
	 * actual form data. The event list provided will be wrapped in a
	...
	/**
	 * Get the root event list for this model. This event list will be constructed
	 * from the form objects value (assumed to be an EventList). Any subclasses that
	 * are installing additional transformed lists should use this method to obtain
	 * the original event list on top of which all the other lists are constructed.
	 */
	 ...
	 * when the value model holding this forms source property has changed (which
	 * can occur when the parent form object is changed). This method is normally
	 * invoked from the parent form object listener.
	 *
	...
	/**
	 * Get the form data we are operating upon. The form object must be castable to
	...
	/**
	 * Indicates that we are creating a new detail object. Default behavior is to
	 * just clear the selection on the master set.
	 */
	...
	protected CommandGroup getCommandGroup() {
		if (commandGroup == null) {
			commandGroup = CommandGroup.createCommandGroup(null,
					new AbstractCommand[] { getDeleteCommand(), getNewFormObjectCommand() });
		}
		return commandGroup;
	}

	/**
	 * Return a standardized row of command buttons, right-justified and all of the
	 * same size, with OK as the default button, and no mnemonics used, as per the
	 ...
	 * override this method in order to call our own createNewDetailObjectCommand
	 * method since the AbstractForm's implementation of createNewFormObjectCommand
	 * is private!
	 */
	@Override
	...
	/**
	 * Create the "new detail object" command. This will encapsulate the action from
	 * our detail forms {@link AbstractForm#getNewFormObjectCommand} as well as
	 * controlling the state of the detail form.
	 *
	 * @return command
	 */
	 ...
	 * <code>masterForm.confirmDelete.message</code>. Subclasses can use the
	 * selected item(s) to construct a more meaningful message.
	 */
	...
	/**
	 * Maybe delete the selected items. If we are configured to confirm the delete,
	 * then do so. If the user confirms, then delete the selected items.
	 */
	protected void maybeDeleteSelectedItems() {
		...
		if (isConfirmDelete()) {
			String title = getMessage(
					new String[] { getId() + ".confirmDelete.title", "masterForm.confirmDelete.title" });
	...
	/**
	 * Deal with the user invoking a "new object" command. If we have unsaved
	 * changes, then we need to query the user to ensure they want to really make
	 * the change.
	 */
	...
	/**
	 * When the results reporter is setup on the master form, we need to capture it
	 * and forward it on to the detail form as well.
	 */
	@Override
	public ValidationResultsReporter newSingleLineResultsReporter(Messagable messageReceiver) {
		...
		ValidationResultsReporter reporter = new SimpleValidationResultsReporter(validationResultsModel,
				messageReceiver);
		return reporter;
	}

	/**
	 * Inner class to handle the list selection and installing the selection into
	 * the detail form.
	 */
	protected class ListSelectionHandler extends ListSelectionListenerSupport {
		...
		protected void maybeChangeSelection(final int newIndex) {
			...
			if (getDetailForm().isDirty()) {
				String title = getMessage(
						new String[] { getId() + ".dirtyChange.title", "masterForm.dirtyChange.title" });
				String message = getMessage(
						new String[] { getId() + ".dirtyChange.message", "masterForm.dirtyChange.message" });
				ConfirmationDialog dlg = new ConfirmationDialog(title, message) {
	...
	/**
	 * Inner class to monitor the editing index on the detail form and update the
	 * state of the delete command whenever it changes.
	 */
	...
	/**
	 * Inner class to monitor the edit state of the detail form and update the state
	 * of the commands whenever it changes.
	 */
	...
	/**
	 * This class handles changes in the property that this master form is editing.
	 * This can occur when the parent form's form object is changed. When that
	 * occurs, our collection data will change automatically and we then need to
	 * update our event list accordingly.
	 */
	...
	/**
	 * Specialized DCBCVM to provide dirty tracking semantics. This will allow the
	 * form model built on this value model to properly track our dirty status.
	 */
	private class DirtyTrackingDCBCVM extends DeepCopyBufferedCollectionValueModel implements DirtyTrackingValueModel {
		 ...
		 * @param wrappedType  the class of the value contained by wrappedModel; this
		 *                     must be assignable to <code>java.util.Collection</code>
		 *                     or <code>Object[]</code>.
		 */
