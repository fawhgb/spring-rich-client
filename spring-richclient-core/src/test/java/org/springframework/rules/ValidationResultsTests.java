...
public class ValidationResultsTests extends SpringRichTestCase {
	...
	public void testValidationResultsCollector() {
		...
		BeanValidationResults r = c.collectResults(rulesSource.getRules(Person.class));
	...
	public void testNestedValidationResultsPropertyConstraint() {
		...
		CompoundConstraint constraint = constraints.or(
				constraints.all("firstName", new Constraint[] { constraints.required(), constraints.minLength(2) }),
				constraints.not(constraints.eqProperty("firstName", "lastName")));
		...
		String message = r.getResults("firstName").buildMessage(Locale.getDefault());
		System.out.println(message);
		assertEquals("First Name must have text and must be at least 2 characters or must *not* equal Last Name.",
				message);
	}
