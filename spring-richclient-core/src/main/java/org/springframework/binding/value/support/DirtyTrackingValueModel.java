...
/**
 * Adds the <code>dirty</code> aspect to a valueModel, tracking changes when
 * needed.
 */
public interface DirtyTrackingValueModel extends ValueModel, PropertyChangePublisher {
	...
	/**
	 * Returns <code>true</code> if value held by this model has changed since the
	 * last call to reset or the last time a value came up from the inner model.
	 */
	boolean isDirty();
