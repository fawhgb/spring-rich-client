...
/**
 * A <code>BufferedValueModel</code> that uses an ObservableList as a buffer to
 * hold chandes to a <code>Collection</code> or <code>array</code>. Internally
 * this is called the "buffered list model."
 * <p>
 ...
 * <li>the new collection is saved into the underlying collection's value
 * model</li>
 * <li>the structure of the list model is compared to the structure of the new
 * underlying collection and if they differ the list model is updated to reflect
...
public class BufferedCollectionValueModel extends BufferedValueModel {
	...
	private boolean hasSameStructure() {
		...
		if (wrappedCollection == null) {
			...
			for (Iterator i = ((Collection) wrappedCollection).iterator(), j = bufferedListModel.iterator(); i
					.hasNext();) {
	 ...
	 * implementations.
	 *
	 * @return ObservableList to use for buffered value. This default uses an
	 *         instance of ListListModel.
	 */
	...
	/**
	 * Gets the list value associated with this value model, creating a list model
	 * buffer containing its contents, suitable for manipulation.
	 * 
		...
		if (wrappedCollection == null) {
			...
			if (wrappedType.isAssignableFrom(wrappedCollection.getClass())) {
				...
				throw new IllegalArgumentException(
						"wrappedCollection must be assignable from " + wrappedType.getName());
			}
	 ...
	 * The default implementation of this method simply returns it. Subclasses can
	 * do whatever is needed to the elements of the colleciton (or the collection
	 * itself). For example, the elements might be cloned or wrapped in a an
	 * adapter.
	 *
	...
	protected boolean hasValueChanged(Object oldValue, Object newValue) {
		return (oldValue == bufferedListModel && newValue == bufferedListModel)
