 ...
 * PropertyAccessStrategy} that exposes JDK 1.5 Annotations for each property in
 * the form of property level user metadata. In order to remain backward
 ...
 * define the same annotation, then the annotation on the getter will override
 * the annotation on the setter.
 *
 * @author andy
...
public class AnnotationAwareBeanPropertyAccessStrategy extends BeanPropertyAccessStrategy {
	...
	protected AnnotationAwareBeanPropertyAccessStrategy(final BeanPropertyAccessStrategy parent,
			final String basePropertyPath) {
	...
	/**
	 * Retrieves annotations for the specified property as user metadata. Uses the
	 * intalled {@link #getAnnotationTranslator annotation translator} to "parse"
	 * annotations to their key + value pair components.
	 * <p/>
	 * Note that currently annotations are retranslated and the returned map is
	 * rebuilt on every invocation of this method. No caching is attempted because
	 * the current use case of this method is a single call for each property during
	 * FormModel construction. If this use case or usage pattern should be changed,
	 * then we may want to consider caching the result of this method.
	 *
	 * @param propertyPath
	 */
	@Override
