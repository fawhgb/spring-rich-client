...
public interface ValidatingFormModel extends ConfigurableFormModel, HierarchicalFormModel {
	...
	/**
	 * Returns the ValidationResultsModel which encapsulates the set of validation
	 * messages currently active against this form model. Will be empty if
	...
	/**
	 * Sets whether or not validation is currently enabled for this form model. If
	 * validation is enabled the form model will immediately validate all form
	 * properties. If validation is disabled all validation messages held by the
	 * ValidationResultsModel will be cleared.
	 */
	void setValidating(boolean validating);

	/**
	 * Forces the form model to validate its self. If validation is disabled it does
	 * nothing.
	 */
	void validate();
	...
	/**
	 * Provide validation messages that are generated by a process separate from the
	 * standard Validator.
	 * <p>
	 * All error messages that are raised using this method must be cleared using
	 * the method {@link #cleanValdationMessage(ValidationMessage)} before the form
	 * model can be commited.
	 *
	...
	/**
	 * Clear validation messages that are generated by a process separate from the
	 * standard Validator.
	 *
