 ...
 * <p>
 * NOTE: This is a framework internal class and should not be instantiated in
 * user code.
 *
...
public class DefaultFieldMetadata extends AbstractPropertyChangePublisher implements FieldMetadata {
	 ...
	 * @param forceReadOnly should readOnly be forced to true; this is required if
	 *                      the property can not be modified. e.g. at the
	 *                      PropertyAccessStrategy level.
	 * @param userMetadata  map using String keys containing user defined metadata.
	 *                      As an example, tiger extensions currently use this to
	 *                      expose JDK 1.5 annotations on the backing object as
	 *                      property metadata. This parameter may be
	 *                      <code>null</code>.
	 */
	public DefaultFieldMetadata(FormModel formModel, DirtyTrackingValueModel valueModel, Class propertyType,
	...
	/**
	 * Sets custom metadata to be associated with this property. A property change
	 * event will be fired (from this FieldMetadata, not from the associated form
	 * property) if <code>value</code> differs from the current value of the
	 * specified <code>key</code>. The property change event will use the value of
	 * <code>key</code> as the property name in the property change event.
	 *
	 * @param key
	 * @param value
	 */
	...
	/**
	 * Clears all custom metadata associated with this property. A property change
	 * event will be fired for every <code>key</code> that contained a non-null
	 * value before this method was invoked. It is possible for a
	 * PropertyChangeListener to mutate user metadata, by setting a key value for
	 * example, in response to one of these property change events fired during the
	 * course of the clear operation. Because of this, there is no guarantee that
	 * all user metadata is in fact completely clear and empty by the time this
	 * method returns.
	 */
	...
	/**
	 * Propagates dirty changes from the value model on to the dirty change
	 * listeners attached to this class.
	 */
