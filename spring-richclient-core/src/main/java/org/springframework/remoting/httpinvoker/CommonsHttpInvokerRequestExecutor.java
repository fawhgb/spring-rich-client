 ...
 * <a href="http://jakarta.apache.org/commons/httpclient">Jakarta Commons
 * HttpClient</a> to execute POST requests. Requires Commons HttpClient 3.0 or
 * higher.
 *
 * <p>
 * Allows to use a pre-configured
 * {@link org.apache.commons.httpclient.HttpClient} instance, potentially with
 * authentication, HTTP connection pooling, etc. Also designed for easy
...
public class CommonsHttpInvokerRequestExecutor extends AbstractHttpInvokerRequestExecutor {
	...
	/**
	 * Create a new CommonsHttpInvokerRequestExecutor with a default HttpClient that
	 * uses a default MultiThreadedHttpConnectionManager. Sets the socket read
	 * timeout to {@link #DEFAULT_READ_TIMEOUT_MILLISECONDS}.
	 * 
	...
	/**
	 * Create a new CommonsHttpInvokerRequestExecutor with the given HttpClient
	 ...
	 * <emphasis>never</emphasis> timeout.
	 * 
	 ...
	 * <p>
	 * This method implements the basic processing workflow: The actual work happens
	 * in this class's template methods.
	 * 
	 ...
	 * specific encoding and potentially set appropriate HTTP request headers.
	 * 
	 * @param config     the HTTP invoker configuration that specifies the target
	 *                   service
	...
	/**
	 * Validate the given response as contained in the PostMethod object, throwing
	 * an exception if it does not correspond to a successful HTTP response.
	 * <p>
	 ...
	 * <p>
	 * The default implementation simply fetches the PostMethod's response body
	 * stream. If the response is recognized as GZIP response, the InputStream will
	 * get wrapped in a GZIPInputStream.
	 * 
	 * @param config     the HTTP invoker configuration that specifies the target
	 *                   service
	 ...
	 * <p>
	 * Default implementation checks whether the HTTP "Content-Encoding" header
