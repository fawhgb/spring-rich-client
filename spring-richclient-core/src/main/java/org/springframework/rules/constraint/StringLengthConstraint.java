...
public class StringLengthConstraint extends AbstractConstraint implements TypeResolvable {
	...
	/**
	 * Constructs a string length constraint with the specified operator and length
	 * constraint.
	 * 
	 ...
	 * @param operator the operator (one of ==, >, >=, <, <=)
	 * @param length   the length constraint
	 * @param type     Type used to resolve messages.
	 */
	public StringLengthConstraint(RelationalOperator operator, int length, String type) {
		...
		this.lengthConstraint = testResultOf(StringLength.instance(), lengthConstraint);
		this.type = type;
	}
	...
	public StringLengthConstraint(int min, int max, String type) {
		...
		this.lengthConstraint = testResultOf(StringLength.instance(), rangeConstraint);
		this.type = type;
	}
