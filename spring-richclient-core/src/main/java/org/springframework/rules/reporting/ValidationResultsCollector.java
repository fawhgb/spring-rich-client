...
public class ValidationResultsCollector {
	...
	public ValidationResults collect(final Object argument, final Constraint constraint) {
	...
	boolean visit(And and) {
		...
		while (it.hasNext()) {
			boolean test = ((Boolean) visitorSupport.invokeVisit(ValidationResultsCollector.this, it.next()))
					.booleanValue();
			if (!test) {
				if (!collectAllErrors) {
	...
	boolean visit(Or or) {
		...
		while (it.hasNext()) {
			boolean result = ((Boolean) visitorSupport.invokeVisit(ValidationResultsCollector.this, it.next()))
					.booleanValue();
			if (result) {
				resultsBuilder.pop(result);
				return true;
	...
	Boolean visit(Not not) {
		...
		Boolean result = (Boolean) visitorSupport.invokeVisit(this, not.getConstraint());
		if (logger.isDebugEnabled()) {
	...
	boolean visit(Constraint constraint) {
		if (logger.isDebugEnabled()) {
			logger.debug("Testing constraint [" + constraint + "] with argument '" + argument + "']");
		}
