...
public class Constraints extends AlgorithmsAccessor {
	 ...
	 * <code>BinaryConstraint</code>. The result is a <code>Constraint</code> which
	 * will test a single variable argument against the constant parameter.
	 * 
	...
	/**
	 * Bind the specified <code>int</code> parameter to the second argument of the
	 * <code>BinaryConstraint</code>. The result is a <code>Constraint</code> which
	 * will test a single variable argument against the constant <code>int</code>
	 * parameter.
	 * 
	 * @param constraint the binary constraint to bind to
	...
	/**
	 * Bind the specified <code>float</code> parameter to the second argument of the
	 * <code>BinaryConstraint</code>. The result is a <code>Constraint</code> which
	 * will test a single variable argument against the constant <code>float</code>
	 * parameter.
	 * 
	 * @param constraint the binary constraint to bind to
	 ...
	 * the <code>BinaryConstraint</code>. The result is a <code>Constraint</code>
	 ...
	 * @return The testing constraint, which on the call to test(o) first evaluates
	 *         'o' using the closure and then tests the result.
	 */
	...
	public PropertyConstraint ifTrue(PropertyConstraint ifConstraint, PropertyConstraint[] thenConstraints) {
		return new ConditionalPropertyConstraint(ifConstraint,
				new CompoundPropertyConstraint(new And(thenConstraints)));
	}
	...
	public PropertyConstraint ifTrue(PropertyConstraint ifConstraint, PropertyConstraint[] thenConstraints,
		...
		return new ConditionalPropertyConstraint(ifConstraint, new CompoundPropertyConstraint(new And(thenConstraints)),
				new CompoundPropertyConstraint(new And(elseConstraints)));
	}
	...
	public PropertyConstraint ifTrue(PropertyConstraint ifConstraint, PropertyConstraint[] thenConstraints,
		...
		return new ConditionalPropertyConstraint(ifConstraint, new CompoundPropertyConstraint(new And(thenConstraints)),
				new CompoundPropertyConstraint(new And(elseConstraints)), type);
	}
	...
	/**
	 * Returns a constraint whose test is determined by a boolean method on a target
	 * object.
	 * 
	...
	/**
	 * Returns a new, empty conjunction prototype, capable of composing individual
	 * constraints where 'ALL' must test true.
	 * 
	 * @return the UnaryAnd
	 */
	...
	/**
	 * Returns a new, empty disjunction prototype, capable of composing individual
	 * constraints where 'ANY' must test true.
	 * 
	 * @return the UnaryOr
	 */
	...
	public PropertyConstraint valueProperties(String propertyName, BinaryConstraint constraint,
	...
	/**
	 * Apply a inclusive "range" constraint between two other properties to a bean
	 * property.
	 * 
	 * @param propertyName    the property with the range constraint.
