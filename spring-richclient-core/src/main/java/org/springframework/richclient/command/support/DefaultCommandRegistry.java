...
/**
 * The default implementation of the {@link CommandRegistry} interface. This
 * implementation may act as the child of another registry, allowing for a
 * hierarchy of registries to be created. If a command is requested from this
 * registry but cannot be found, the request will be delegated to the parent
 * registry.
 *
 *
 * @author Keith Donald
...
public class DefaultCommandRegistry implements CommandRegistry, CommandRegistryListener {
	...
	/**
	 * Creates a new {@code DefaultCommandRegistry} as a child of the given
	 * registry. The newly created instance will be added as a listener on the
	 * parent registry.
	 *
	...
	/**
	 * Sets the given registry to be the parent of this instance. If the new parent
	 * is not null, this instance will act as a registry listener on it.
	 *
	 * @param parent The parent registry. May be null.
	 */
	...
	/**
	 * This method is provided as a private helper so that it can be called by the
	 * constructor, instead of the constructor having to call the public overridable
	 * setParent method.
	 */
	...
	public ActionCommand getActionCommand(String commandId) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Attempting to retrieve ActionCommand with id [" + commandId + "] from the command registry.");
		}
	...
	public CommandGroup getCommandGroup(String groupId) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Attempting to retrieve command group with id [" + groupId + "] from the command registry.");
		}
	...
	public void registerCommand(AbstractCommand command) {
		...
		org.springframework.util.Assert.notNull(command, "Command cannot be null.");
		org.springframework.util.Assert.isTrue(command.getId() != null,
		...
		if (previousCommand != null && logger.isWarnEnabled()) {
			logger.warn("The command [" + previousCommand + "] was overwritten in the registry with the command ["
					+ command + "]");
		}
	...
	/**
	 * Fires a 'commandRegistered' {@link CommandRegistryEvent} for the given
	 * command to all registered listeners.
	 *
	...
	public void setTargetableActionCommandExecutor(String commandId, ActionCommandExecutor executor) {
		...
		TargetableActionCommand command = (TargetableActionCommand) getCommand(commandId,
