...
/**
 * An action command for displaying a {@link Page} based on a provided
 * {@link PageDescriptor}.
 */
public class ShowPageCommand extends ApplicationWindowAwareCommand implements InitializingBean {
	...
	/**
	 * Creates a new uninitialized {@code ShowPageCommand}. The
	 * {@code applicationWindow} and {@code pageDescriptor} properties must be set
	...
	/**
	 * Creates a new {@code ShowPageCommand} with the given page descriptor and
	 * associated application window. The new instance will have a command
	 * identifier equal to the id from the page descriptor, the command will be
	 * enabled by default.
	 *
	 * @param pageDescriptor    The object describing the page that this command
	 *                          will be responsible for showing.
	 * @param applicationWindow The application window that the command belongs to.
	 *
	 * @throw IllegalArgumentException if {@code pageDescriptor} or
	 ...
	 * This command object will be assigned the id, label, icon, and caption from
	 * the given page descriptor.
	 *
	...
	protected void doExecuteCommand() {
		// FIXME getApplicationWindow can potentially return null. This should probably
		// be
		// made an invariant on the ApplicationWindowAwareCommand, that it never returns
		// null.
