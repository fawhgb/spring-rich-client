...
/**
 * A {@link GroupMember} implementation that can be used as a placeholder for
 * lazily initialized commands.
 *
 */
public class LazyGroupMember extends GroupMember {
	...
	/**
	 * Creates a new {@code LazyGroupMember} belonging to the given command group
	 * and managing a lazily initialized command with the given id.
	 *
	...
	public LazyGroupMember(CommandGroup parentGroup, String lazyCommandId) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug(
					"Lazy group member '" + lazyCommandId + "' instantiated for group '" + parentGroup.getId() + "'");
		}
	...
	/**
	 * Delegates this call to the lazily loaded member, but only if it has already
	 * been loaded. Calling this method before the underlying member has beeen
	...
	/**
	 * Attempts to load the lazy command from the command registry of the parent
	 * command group, but only if it hasn't already been loaded.
	 */
	private void loadIfNecessary() {
		...
		org.springframework.util.Assert.isTrue(parentGroup.getCommandRegistry() != null,
				"Command registry must be set for group '" + parentGroup.getId() + "' in order to load lazy command '"
						+ lazyCommandId + "'.");
