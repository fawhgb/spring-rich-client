...
/**
 * An immutable parameter object consisting of the text, mnemonic character,
 * mnemonic character index and keystroke accelerator that may be associated
 * with a labeled command button.
 *
 * <p>
 * This class also acts as a factory for creating instances of itself based on a
 * string descriptor. The syntax used for this descriptor is described in the
 * javadoc for the {@link #valueOf(String)} method.
 *
 *
 * @author Keith Donald
...
public final class CommandButtonLabelInfo implements ButtonConfigurer {
	...
	/**
	 * Return an instance of this class, created by parsing the information in the
	 * given label descriptor string. The expected format of this descriptor is the
	 * same as that used by the {@link LabelInfo} class, with the following
	 * additions:
	 *
	 * <ul>
	 * <li>The @ symbol is an escapable character.</li>
	 * <li>Everything after the first unescaped @ symbol will be treated as the
	 * textual representation of the keystroke accelerator.</li>
	 * </ul>
	 *
	 * The expected format of the keystroke accelerator string is as described in
	 * the javadocs for the {@link KeyStroke#getKeyStroke(String)} method.
	 *
	 *
	 * @param labelDescriptor The label descriptor. May be null or empty, in which
	 *                        case, a default blank label info will be returned.
	 * @return A CommandButtonLabelInfo instance, never null.
	 *
	 * @throws IllegalArgumentException if {@code labelDescriptor} contains invalid
	 *                                  syntax.
	 *
	 * @see LabelInfo
	 * @see KeyStroke
			...
			if (currentChar == '\\') {
				...
				if (i + 1 == labelDescriptor.length()) {
					throw new IllegalArgumentException("The label descriptor [" + labelDescriptor
							+ "] does not specify a valid accelerator after the last " + "non-espaced @ symbol.");
				}
				...
				if (keyStroke == null) {
					throw new IllegalArgumentException("The keystroke accelerator string [" + acceleratorStr
							+ "] from the label descriptor [" + labelDescriptor + "] is not a valid keystroke format.");
				}
	...
	/**
	 * Creates a new {@code CommandButtonLabelInfo} that will display the given text
	 * on its label. There will be no associated mnemonic character and no keystroke
	 * accelerator.
	 *
	...
	/**
	 * Creates a new {@code CommandButtonLabelInfo} with the given label information
	...
	/**
	 * Returns the zero-based index for the mnemonic character within the label
	 * text.
	 *
	 * @return The mnemonic index or -1 to indicate that there is no associated
	 *         mnemonic character.
	 */
	...
	public boolean equals(Object obj) {
		...
		if (!this.labelInfo.equals(other.labelInfo)
				|| !ObjectUtils.nullSafeEquals(this.accelerator, other.accelerator)) {
			return false;
		}
	...
	public String toString() {
		return new ToStringCreator(this).append("labelInfo", this.labelInfo).append("accelerator", this.accelerator)
				.toString();
	}
