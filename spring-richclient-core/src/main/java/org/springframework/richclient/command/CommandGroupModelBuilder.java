...
public abstract class CommandGroupModelBuilder {
	 ...
	 * 
	 * @param commandGroup at the root of the structure
	 ...
	 * the structure should continue deeper down.
	 * 
	 * Implementations can decide based on the current visited commandGroup and the
	 * level information.
	 * 
	 * Default implementation at this abstract class level, is to keep on going down
	 * the structure. (subclasses should only override when needing to change that.)
	 * 
	 * @param commandGroup currently visited.
	 * @param level        in the structure we are at ATM
	 ...
	 * 
	 * @param parentModel the objectmodel that was created for the parent-command.
	 * @param command     currently visited command in the structure.
	 * @param level       in the structure we are at ATM
	 * @return the top level object model to build.
	 */
	 ...
	 * 
	 * Since the parentModel is also passed in, this implementation can use it to
	 * downcast and decide to hook up the new client-structure.
	 * 
	 * In a same manner the object-structure returned by this method will be passed
	 * down in the tree as the parentModel for nested nodes.
	 * 
	 * In general, if an implementation subclass is not building extra stuff for a
	 * particular command at a particular level, then it is generally wise to just
	 * pass down the parentModel.
	 * 
	 * @param parentModel the objectmodel that was created for the parent-command.
	 * @param command     currently visited command in the structure.
	 * @param level       in the structure we are at ATM
	 ...
	 * passed in command-group structure by performing the appropriate callbacks to
	 * the subclass implementations of {@link #buildRootModel(CommandGroup)},
	 * {@link #buildChildModel(Object, AbstractCommand, int)}, and
	 * {@link #buildGroupModel(Object, CommandGroup, int)}.
	 * 
	 * Additionally,
	 * 
	 * @param commandGroup the root of the structure for which an mapping
	 *                     objectmodel will be built.
