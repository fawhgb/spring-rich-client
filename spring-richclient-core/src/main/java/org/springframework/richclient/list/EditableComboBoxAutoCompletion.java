...
/**
 * Provides auto-completion for an editable combobox. Based on public domain
 * postings. Original author unknown. Also copied some code from
 * {@link ComboBoxAutoCompletion} to deal with focus loss.
 *
...
public class EditableComboBoxAutoCompletion extends KeyAdapter {
	...
	/**
	 * Handle a key release event. See if what they've type so far matches anything
	 * in the selectable items list. If so, then show the popup and select the item.
	 * If not, then hide the popup.
	 * 
	 * @param e key event
	 */
	@Override
