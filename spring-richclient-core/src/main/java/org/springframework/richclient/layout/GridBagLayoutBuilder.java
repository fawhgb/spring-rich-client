...
public class GridBagLayoutBuilder implements LayoutBuilder {
	...
	/**
	 * Appends the given component to the end of the current line, using the default
	 * insets and no expansion
	 *
			...
			for (int i = 1; i < (y + rowSpan); i++) {
				...
				if (row.get(col) != null) {
					...
					throw new IllegalStateException(
							"Trying to overwrite another component: " + component + ", " + col + " " + y);
				}
	...
	/**
	 * Appends the given label to the end of the current line. The label does not
	 * "grow."
	 *
	...
	/**
	 * Appends a seperator (usually a horizonal line) using the provided string as
	 * the key to look in the {@link #setComponentFactory(ComponentFactory)
	 * ComponentFactory's}message bundle for the text to put along with the
	 * seperator. Has an implicit {@link #nextLine()}before and after it.
	 *
	 * @return "this" to make it easier to string together append calls
	 */
	...
	private void addRow(final List row, final int currentRowIndex, final int lastRowIndex, final JPanel panel) {
	...
	/**
	 * Should the last column before a {@link #nextLine()}automaticly span to the
	 * end of the panel?
	 * <p />
	 *
