 ...
 * table.row().separator(&quot;General 1&quot;).row().cell(new JLabel(&quot;Company&quot;), &quot;colSpec=right:pref colGrId=labels&quot;)
 * 		.labelGapCol().cell(new JFormattedTextField()).row().cell(new JLabel(&quot;Contact&quot;))
 * 		.cell(new JFormattedTextField()).unrelatedGapRow().separator(&quot;Propeller&quot;).row().cell(new JLabel(&quot;PTI [kW]&quot;))
 * 		.cell(new JFormattedTextField()).unrelatedGapCol()
 * 		.cell(new JLabel(&quot;Description&quot;), &quot;colSpec=right:pref colGrId=labels&quot;).labelGapCol()
 * 		.cell(new JScrollPane(new JTextArea()), &quot;rowspan=3&quot;).row().cell(new JLabel(&quot;R [mm]&quot;))
 * 		.cell(new JFormattedTextField()).cell().row().cell(new JLabel(&quot;D [mm]&quot;)).cell(new JFormattedTextField())
 * 		.cell();
 * table.getPanel();
 * </pre>
 *
...
public class TableLayoutBuilder implements LayoutBuilder {
	...
	/**
	 * Creates a new TableLayoutBuilder which will perform it's layout in the
	 * supplied JPanel. Note that any components that are already contained by the
	...
	/**
	 * Sets the {@link ComponentFactory}that this uses to create things like labels.
	 */
	...
	/**
	 * Returns the current row (zero-based) that the builder is putting components
	 * in.
	 */
	...
	/**
	 * Inserts a new row. A gap row with specified RowSpec will be inserted before
	 * this row.
	 */
	...
	/**
	 * Inserts a new row. A related component gap row will be inserted before this
	 * row.
	 */
	...
	/**
	 * Inserts a component at the current row/column. Attributes may be zero or more
	 * of rowSpec, columnSpec, colGrId, rowGrId, align and valign.
	 */
	 ...
	 * 
	 * @deprecated this is a layout builder, creating components should be done
	 *             elsewhere, use cell() methods instead
	 */
	@Deprecated
	...
	/**
	 * Creates and returns a JPanel with all the given components in it, using the
	 * "hints" that were provided to the builder.
	 * 
	 ...
	 *
	 * @param order forcus traversal order. Must be one of
	 *              {@link #COLUMN_MAJOR_FOCUS_ORDER} or
	 *              {@link #ROW_MAJOR_FOCUS_ORDER}.
	 */
			...
			while (st.nextToken() != StreamTokenizer.TT_EOF) {
				if (name == null && st.ttype == StreamTokenizer.TT_WORD) {
					...
					needEquals = false;
						...
						&& (st.ttype == StreamTokenizer.TT_WORD || st.ttype == '\'' | st.ttype == '"')) {
