...
/**
 * <p>
 * PropertyColumnTableDescription is the default implementation of
 * {@link TableDescription} which uses class introspection to create columns.
 * Each column is based on a property of the class and thus should be accessible
 * through an isXXX or getXXX method.
 * </p>
 *
 * <p>
 * Additionally writing capabilities are available by providing a specific
 * {@link javax.swing.table.TableCellEditor}. When using this table description
 * with a detail form and a backing formModel, take a look at the
 * {@link ValueModelTableCellEditor}.
 * </p>
 *
 * <p>
 * Nested properties can be accessed using the dot notation:
 * 'propertyA.propertyB'. At any point in this chaining getting a null value
 * will stop evaluating the expression and returns null.
 * </p>
 *
 ...
 *
 * <p>
 * More possibilities and additional addXXX methods can be found in the
 * {@link PropertyColumn} class.
 * </p>
 *
 * NOTE: this class provides a number of addPropertyColumn(...) methods to add
 * columns. As there are many features on a column, this led to a huge number of
 * these constructions. To simplify the {@link PropertyColumnTableDescription}
 * class and the adding of new features, a new way of column creation has been
 * implemented with the addXXX methods on the {@link PropertyColumn} class. New
 * features will be exclusive to the addXXX methods.
 *
 */
public class PropertyColumnTableDescription implements TableDescription {
	...
	/**
	 * Comparator to use as default (when list is set or other specific sorting is
	 * removed).
	 */
	private Comparator defaultComparator;

	/**
	 * A table can contain one column with checkboxes that holds a selection. At the
	 * moment the specific table implementation is responsible of filtering the
	 * table rows and checking which rows are selected.
	 */
	...
	/**
	 * Constructor creating a {@link PropertyColumnTableDescription} based on the
	 * given type. Each column created will need to have a corresponding property
	 * path on this type.
	 *
	 * @param id                id used to fetch messages for the column headers.
	 * @param forEntityType     type of the row item.
	 * @param numberOfColumns   expected number of columns.
	 * @param defaultComparator comparator for the default sorting. Initial table
	 *                          will be sorted according to this comparator as well
	 *                          as when all other specific sorting is removed.
	 *
	 */
	public PropertyColumnTableDescription(final String id, Class forEntityType, int numberOfColumns,
	...
	/**
	 * Create and add a column for the given property. Property type is passed or
	 * determined (when <code>null</code>) by examining the {@link Accessor} and
	 * headerKeys are added based upon the id of the
	 * {@link PropertyColumnTableDescription}, the propertyName and the postfix
	 * "HEADER".
	 *
	 * @param propertyName name of the property.
	 * @param propertyType type of the property. If <code>null</code> a type will be
	 *                     determined by examining the accessor method.
	 */
	...
	public void setPropertyColumns(Collection<PropertyColumn> propertyColumns) {
		...
		for (PropertyColumn propertyColumn : columns) {
			Accessor accessorForProperty = ClassUtils.getAccessorForProperty(entityClass,
					propertyColumn.getPropertyName());
			...
			propertyColumn.setHeaderKeys(
					RcpSupport.getMessageKeys(this.id, propertyColumn.getPropertyName(), RcpSupport.HEADER));
		}
	}
	...
	/**
	 * WARNING: propertyType is discarded, it should be fetched from the entityType
	 ...
	 */
	@Deprecated
	public void addPropertyColumn(String propertyName, Class propertyType, int width, TableCellRenderer renderer,
			boolean visible) {
		addPropertyColumn(propertyName).withFixedWidth(width).withRenderer(renderer).withVisible(visible);
	}
	...
	public void addPropertyColumn(String propertyName, Class propertyType, String[] headerKeys, int minWidth,
		...
		addPropertyColumn(propertyName).withHeaderKeys(headerKeys).withMinWidth(minWidth).withMaxWidth(maxWidth)
				.withResizable(resizable).withRenderer(renderer).withComparator(comparator)
				.withFilterColumn(isInTextFilter);
	}
	...
	public void addSelectPropertyColumn(String propertyName, Comparator<?> comparator) {
		addSelectPropertyColumn(propertyName, PropertyColumn.UNSPECIFIED_WIDTH, PropertyColumn.UNSPECIFIED_WIDTH,
				comparator);
	}
	...
	public void addSelectPropertyColumn(String propertyName, int minWidth, int maxWidth, Comparator comparator) {
		addSelectPropertyColumn(propertyName, RcpSupport.getMessageKeys(this.id, propertyName, RcpSupport.HEADER),
				minWidth, maxWidth, true, comparator);
	}
	...
	public void addSelectPropertyColumn(String propertyName, String[] headerKeys, int minWidth, int maxWidth,
		...
		if (hasSelectColumn) {
			throw new IllegalArgumentException(
					"Already a selectColumn specified, cannot set " + propertyName + " as selectColumn");
		}
		...
		columns.add(0, new PropertyColumn(Boolean.class, propertyAccessor, propertyName, headerKeys, minWidth, maxWidth,
				resizable, null, new DefaultCellEditor(cellEditorComponent), comparator, true, true));
	}
	...
	public void addSelectPropertyColumn(String propertyName, String[] headerKeys, int minWidth, int maxWidth,
		...
		cellEditorComponent.setHorizontalAlignment(SwingConstants.CENTER);
		columns.add(0, new PropertyColumn(Boolean.class, propertyAccessor, propertyName, headerKeys, minWidth, maxWidth,
				resizable, null, new DefaultCellEditor(cellEditorComponent), comparator, true, visible));
	}
	...
	/**
	 * Set the comparator to use as default (when table is filled or other specific
	 * sorting is removed).
	 */
	 ...
	 *
	 * @param propertyIndex column index.
	...
	public void setValue(Object rowObject, int propertyIndex, Object newValue) {
		try {
			...
			log.warn("Error writing property " + propertyIndex + " to object " + rowObject + " new value: " + newValue,
					e);
			throw new RuntimeException(
					"Error writing property " + propertyIndex + " to object " + rowObject + " new value: " + newValue,
					e);
		}
	}
