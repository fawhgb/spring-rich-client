...
public abstract class AbstractDataEditorWidget extends AbstractTitledWidget implements TitledWidget, SelectionWidget {
	...
	public static final RowSpec FILL_ROW_SPEC = new RowSpec(RowSpec.FILL, Sizes.PREFERRED, FormSpec.DEFAULT_GROW);
	...
	/**
	 * Creates the observer that listens to selections in the listView. Normally
	 * forwards the selection to the detailForm.
	 */
	...
	protected final JComponent createDataEditorWidget() {
		...
		if (isSelectMode() == ON) {
			...
			getTableWidget().getTable().getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
	...
	/**
	 * Convenience method to retrieve the action command that should be used when
	 * changes are made in the detailForm. This can be update or add, according to
	 * whether a row is selected and changed or no row is selected.
	 *
	...
	protected void doUpdate() {
		...
		try {
			...
			// set the changes back on the model if the object could be set on the form
			// model
			if (success) {
				RcpSupport.mapObjectOnFormModel(getDetailForm().getFormModel(), changedObject);
			}
			throw e;
		}
	}
	...
	protected void doCreate() {
		...
		try {
			...
			throw e;
			// make form dirty, show error in messagepane and throw exception to display
			// error dialog
		}
	}
	...
	/**
	 * Returns the commandGroup that should be used to create the popup menu for the
	 * table.
	 */
	protected CommandGroup getTablePopupMenuCommandGroup() {
		return CommandGroup.createCommandGroup(new Object[] { getEditRowCommand(), "separator", getAddRowCommand(),
				getCloneRowCommand(), getRemoveRowsCommand(), "separator", getRefreshCommand(), "separator",
	...
	private JComponent getFilterPanel() {
		...
		ColumnSpec[] columnSpecs = new ColumnSpec[] { FILL_COLUMN_SPEC };
		RowSpec[] rowSpecs = new RowSpec[] { new RowSpec(RowSpec.FILL, Sizes.DEFAULT, FormSpec.DEFAULT_GROW), // form
																												// gewrapped
																												// in
																												// een
																												// scrollpane.
	...
	private JComponent getFilterControlPanel() {
		CommandGroup controlCommands = CommandGroup
				.createCommandGroup(new Object[] { getExecuteFilterCommand(), getEmptyFilterCommand() });
	...
	private JComponent getTableFilterControlPanel() {
		...
		ColumnSpec[] columnSpecs = getTableColumnSpecs(
				tableFilterControlCommands != null && tableFilterControlCommands.size() > 0);
		RowSpec[] rowSpecs = new RowSpec[] { FILL_ROW_SPEC };
		FormLayout formLayout = new FormLayout(columnSpecs, rowSpecs);
		JPanel buttonPanel = new JPanel(formLayout);
		int columnCounter = 1;
	...
	protected void removeRows() {
		...
		for (int i = 0; i < selectedRows.length && (answer == JOptionPane.YES_OPTION); i++) {
			...
			try {
				...
				if (remaining > 0) {
					String ttl = RcpSupport.getMessage(getId(), REMOVE_CONTINUE_AFTER_ERROR, RcpSupport.TITLE);
					String errMsg = RcpSupport.getMessage(getId(), REMOVE_CONTINUE_AFTER_ERROR, RcpSupport.TEXT,
							new Object[] { Integer.valueOf(remaining) });
					answer = JOptionPane.showConfirmDialog(getComponent(), errMsg, ttl, JOptionPane.YES_NO_OPTION,
			...
			private String createTabDelimitedSelectedRowsContent() {
				...
				for (int rowIndex : jxTable.getSelectedRows()) {
					...
					for (TableColumn tableColumn : jxTable.getColumns()) {
						...
						Component component = renderer.getTableCellRendererComponent(jxTable, unformattedValue, false,
								false, rowIndex, columnViewIndex);
	...
	protected AbstractCommand[] getControlCommands() {
		...
		return new AbstractCommand[] { getCloseCommand()
				// close
		};
	}
	...
	/**
	 * Fetch the detailed object from the back-end. If the baseObject is already
	 * detailed, the baseObject can be returned directly if and only if no forceLoad
	 * is requested. This logic is also apparent in the
	 * {@link org.springframework.richclient.widget.editor.provider.DataProvider}
	 * class.
	 *
	 * @param baseObject object containing enough information to fetch a detailed
	 *                   version.
	 * @param forceLoad  if <code>true</code> always load the detailed object from
	 *                   the back-end, if <code>false</code> a shortcut can be
	 *                   implemented by returning the baseObject directly.
	...
	public boolean canClose() {
		...
		if (detailFormModel.isEnabled() && detailFormModel.isDirty()) {
			...
			{
				answer = RcpSupport.showWarningDialog(getComponent(), UNSAVEDCHANGES_UNCOMMITTABLE_WARNING_ID,
						JOptionPane.YES_NO_OPTION);
				// the following might seem strange, but it aligns the answer with the other
				// part of this if construction
				// if we said 'yes keep editing': don't discard changes, continue editing to
				// save it later on == CANCEL in previous if
				// if we said 'no discard changes': discard changed and switch to other row ==
				// NO in previous if
	...
	private boolean setDetailFormObject(Object rowObject, Observer reportingObserver, boolean forceLoad) {
		...
		if (rowObject != null) {
			...
			// if null, remove from list, set rowObject null and fall through (exception
			// will be displayed)
	...
	private void enableSelectButton(Object selection) {
		if (selectionCommand != null) {
			if (selection == null) {
				...
				selectionCommand.setEnabled(!(selection instanceof Object[]) || (((Object[]) selection).length == 1));
			}
		}
	}
	...
	private class ListSelectionObserver implements Observer {
		...
		public void update(Observable o, Object rowObject) {
			AbstractDataEditorWidget.this.setDetailFormObject(rowObject instanceof Object[] ? null : rowObject,
					tableSelectionObserver, false);
