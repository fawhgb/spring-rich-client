...
public class DefaultDataEditorWidget extends AbstractDataEditorWidget
	...
	/**
	 * {@link org.springframework.binding.validation.ValidationResultsModel}
	...
	/**
	 * {@link SwingWorker} which retrieves list from back-end and fills table with
	 * result.
	 * <p/>
		...
		protected void done() {
			try {
				...
				if (e.getCause() instanceof MaximumRowsExceededException) {
					...
					maximumRowsExceededMessage.setMessage(getMessage("MaximumRowsExceededException.notice",
							new Object[] { mre.getNumberOfRows(), mre.getMaxRows() }));
	...
	/**
	 * This method is called on the gui-thread when the worker ends. As default it
	 * will check for the PARAMETER_DEFAULT_SELECTED_OBJECT parameter in the map.
	 *
	 ...
	 * {@link org.springframework.richclient.form.Form}s and listView later.
	 *
	...
	/**
	 * Constructor with id and {@link DataProvider}. Add
	 * {@link org.springframework.richclient.form.Form}s and listView later.
	 *
	 ...
	 */
	public DefaultDataEditorWidget(String id, DataProvider provider, AbstractForm form, TableDescription tableDesc,
			FilterForm filterForm) {
		setId(id);
	 ...
	 * </ul>
	 *
	 * @param parameters a number of parameters that can influence this run. Should
	 *                   be a non-modifiable map or a specific instance.
	 */
	@Override
	public synchronized void executeFilter(Map<String, Object> parameters) {
		if (listWorker == null) {
			...
			log.debug("Execute Filter with criteria: " + listWorker.filterCriteria + " and parameters: " + parameters);
			listWorker.execute();
		}
	}
	 ...
	 * <p/>
	 * <p>
	 * Alternative: {@link #executeFilter()} will launch separate worker and fills
	 * table.
	 * </p>
	 *
	 * @param criteria
	...
	protected List getList(Object criteria) {
		...
		try {
			...
			setRows(Collections.EMPTY_LIST);
			setMessage(new DefaultMessage(getMessage("MaximumRowsExceededException.notice",
					new Object[] { mre.getNumberOfRows(), mre.getMaxRows() }), Severity.WARNING));
			if (getToggleFilterCommand() != null) {
				getToggleFilterCommand().doShow();
			}
			return null;
	...
	protected void newRow(Object newClone) {
		if (newClone == null) {
			super.newRow(
					getDataProvider().newInstance(getFilterForm() == null ? null : getFilterForm().getFormObject()));
		} else {
