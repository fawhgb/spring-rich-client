...
/**
 * TableWidget describes the needed functionality for widgets that produce
 * tables.
 *
 * The TableWidget manages a data-list of objects whose properties are shown in
 * one or more different columns of the table.
 *
 * Some methods of TableWidget (remove, select) assume correct implementation of
...
public interface TableWidget<T> extends Widget {
	 ...
	 *
	 * @param newRows The list of row objects
	 */
	void setRows(Collection<T> newRows);

	/**
	 * Returns the current list of objects behind the table. It does not take into
	 ...
	 *
	 * @param rows The collection of objects to add
	 */
	 ...
	 * observers. The second argument allows 1 observer to be excluded from the
	 * notification (needed if the observer is the caller).
	 *
	 * @param toPointTo           row-object that needs to be selected
	 * @param originatingObserver Optional observer that doesn't want to be notified
	 *                            of the selection
	 ...
	 *
	 * @param index               index of the row that needs to be selected
	 * @param originatingObserver Optional observer that doesn't want to be notified
	 *                            of the selection
	 */
	 ...
	 *
	 * @param rows                Array with row objects
	 * @param originatingObserver Optional observer that doesn't want to be notified
	 *                            of the selection
	 */
	 ...
	 * @param newObject           The replacing object
	 * @param originatingObserver Optional observer that doesn't want to be notified
	 *                            of the replacement
	 */
	 ...
	 *
	 * @param oldObject The collection of objects to be replaced
	 * @param newObject The replacing collection of objects
	 */
	...
	/**
	 * Registers a listener that will receive events on selection of an object in
	 * the table.
	 *
	...
	/**
	 * Returns the table, getComponent() does not need to return only table (a
	 * scrollpane can be returned too containing the table), as this method will
	 * enable you to get to the table.
	 *
	 * @return The table
	...
	/**
	 * @return A textfield that enables full-text filtering n the contents of the
	 *         table or null if this feature is not supported
	 */
	JTextField getTextFilterField();

	/**
	 * @return A label on which the index, the selected row count and the row count
	 *         is shown
	 */
