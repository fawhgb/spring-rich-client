 ...
 * Timeouts are supported. Deprecated in favor of javax.swing.SwingWorker.
 */
@Deprecated
public abstract class SwingWorker extends FutureResult implements Runnable {
	 ...
	 * maximum time to wait for the worker to complete. There is no time limit if
	 * the timeout is <code>0</code> (default).
	 */
	...
	public synchronized void interrupt() {
		if (thread != null) {
			/*
			 * Try-catch is workaround for JDK1.2.1 applet security bug. JDK1.2.1
			 * overzealously throws a security exception if an applet interrupts a thread
	...
	/**
	 * Clears the worker thread variable and the FutureResult state, allowing this
	 * SwingWorker to be reused. This is not particularly recommended and must be
	 * done only when you know that the worker thread is finished and that no other
	 * object is depending on the properties of this SwingWorker.
	 */
	@Override
