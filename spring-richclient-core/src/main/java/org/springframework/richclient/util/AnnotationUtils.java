...
public class AnnotationUtils {
	...
	/**
	 * Retrieves all annotations present on the specified property. This method will
	 * gather the annotations present on both the getter and the setter of the
	 * property. If a particular Annotation type appears on both the getter and
	 * setter, then the instance appearing on the Getter will override (replace) the
	 * instance appearing on the Setter.
	 *
	 * @param property
	 * @return collection containing all annotations present on the specified
	 *         property. This method will never return <code>null</code>, but will
	 *         return an empty Collection if no annotations appear on the property.
	 */
