...
public class EventListenerListHelper implements Serializable {
	...
	private static final Map methodCache = new CachingMapDecorator() {
		 ...
		 * {@link MethodCacheKey}. The value to be created will be a {@link Method}
		 * object that is specified by the given key.
		 *
		 * @param o The key that the newly created object will be stored under. This is
		 *          expected to be an instance of {@link MethodCacheKey} that contains
		 *          the class, method name and number of parameters of the
		 *          {@link Method} to be created.
		 *
		 * @throws ClassCastException       if {@code o} can not be assigned to
		 *                                  {@link MethodCacheKey}.
		 * @throws IllegalArgumentException if the listener class specified by
		 *                                  {@code o}, does not have an implementation
		 *                                  of the method specified in the given key.
		 */
		@Override
		protected Object create(Object o) {
			...
			for (int i = 0; i < methods.length; i++) {
				...
				if (method.getName().equals(key.methodName) && method.getParameterTypes().length == key.numParams) {
					if (fireMethod != null) {
						throw new UnsupportedOperationException(
								"Listener class [" + key.listenerClass + "] has more than 1 implementation of method ["
										+ key.methodName + "] with [" + key.numParams + "] parameters.");
					}
					fireMethod = method;
				}
			}
			...
			if (fireMethod == null) {
				throw new IllegalArgumentException(
						"Listener class [" + key.listenerClass + "] does not implement method [" + key.methodName
	...
	/**
	 * Create new <code>EventListenerListHelper</code> instance that will maintain a
	 * list of event listeners of the given class.
	 *
	 * @param listenerClass The class of the listeners that will be maintained by
	 *                      this list helper.
	 *
	...
	/**
	 * Returns an array of all the listeners registered with this list. This method
	 * is intended for use in subclasses that require the fastest possible access to
	 * the listener list. It is recommended that unless performance is absolutely
	 * critical access to the listener list should be through the
	 * <code>iterator</code>,<code>forEach</code> and <code>fire</code> methods
	 * only.
	 * <p>
	 * NOTE: The array returned by this method is used internally by this class and
	...
	/**
	 * Returns an iterator over the list of listeners registered with this list. The
	 * returned iterator does not allow removal of listeners. To remove a listener,
	 * use the {@link #remove(Object)} method.
	 *
	...
	/**
	 * Invokes the method with the given name and no parameters on each of the
	 * listeners registered with this list.
	 *
	 * @param methodName the name of the method to invoke.
	 *
	 * @throws IllegalArgumentException if no method with the given name and an
	 *                                  empty parameter list exists on the listener
	 *                                  class maintained by this list helper.
	 */
	...
	/**
	 * Invokes the method with the given name and a single parameter on each of the
	 * listeners registered with this list.
	 *
	 * @param methodName the name of the method to invoke.
	 * @param arg        the single argument to pass to each invocation.
	 *
	 * @throws IllegalArgumentException if no method with the given name and a
	 *                                  single formal parameter exists on the
	 *                                  listener class managed by this list helper.
	 */
	...
	/**
	 * Invokes the method with the given name and two parameters on each of the
	 * listeners registered with this list.
	 *
	 * @param methodName the name of the method to invoke.
	 ...
	 *
	 * @throws IllegalArgumentException if no method with the given name and 2
	 *                                  formal parameters exists on the listener
	 *                                  class managed by this list helper.
	 */
	...
	/**
	 * Invokes the method with the given name and number of formal parameters on
	 * each of the listeners registered with this list.
	 *
	 * @param methodName the name of the method to invoke.
	 * @param args       an array of arguments to pass to each invocation.
	 *
	 * @throws IllegalArgumentException if no method with the given name and number
	 *                                  of formal parameters exists on the listener
	 *                                  class managed by this list helper.
	 */
	...
	/**
	 * Adds <code>listener</code> to the list of registered listeners. If listener
	 * is already registered this method will do nothing.
	 *
	 * @param listener The event listener to be registered.
	 *
	 * @return true if the listener was registered, false if {@code listener} was
	 *         null or it is already registered with this list helper.
	 *
	 * @throws IllegalArgumentException if {@code listener} is not assignable to the
	 *                                  class of listener that this instance
	 *                                  manages.
	 */
	...
	/**
	 * Adds all the given listeners to the list of registered listeners. If any of
	 * the elements in the array are null or are listeners that are already
	 * registered, they will not be registered again.
	 *
	 * @param listenersToAdd The collection of listeners to be added. May be null.
	 *
	 * @return true if the list of registered listeners changed as a result of
	 *         attempting to register the given collection of listeners.
	 *
	 * @throws IllegalArgumentException if any of the listeners in the given
	 *                                  collection are of a type that is not
	 *                                  assignable to the class of listener that
	 ...
	 *
	 * @throws IllegalArgumentException if {@code listener} is null or not
	 *                                  assignable to the class of listener that is
	...
	/**
	 * Invokes the method with the given name on each of the listeners registered
	 * with this list helper. The given arguments are passed to each method
	 * invocation.
	 *
	 * @param methodName The name of the method to be invoked on the listeners.
	 * @param eventArgs  The arguments that will be passed to each method
	 *                   invocation. The number of arguments is also used to
	 *                   determine the method to be invoked.
	 *
	 * @throws EventBroadCastException if an error occurs invoking the event method
	...
	/**
	 * Indicates that an error has occurred attempting to broadcast an event to
	 * listeners.
	 */
	public static class EventBroadcastException extends NestedRuntimeException {
		...
		/**
		 * Creates a new {@code EventBroadcastException} with the given detail message
	...
	private void checkListenerType(Object listener) {
		if (!listenerClass.isInstance(listener)) {
			throw new IllegalArgumentException(
					"Listener [" + listener + "] is not an instance of [" + listenerClass + "].");
		}
	}
	...
	/**
	 * Returns an object which is a copy of the collection of listeners registered
	 * with this instance.
	 *
