 ...
 *
 * @author Eric Albert (
 *         <a href="mailto:ejalbert@cs.stanford.edu">ejalbert@cs.stanford.edu
 *         </a>)
...
public class BrowserLauncher {
		...
		if (osName.startsWith("Mac OS")) {
			...
			try {
				...
				if (version == 2) {
					...
					// 2.1 actually works via Runtime.exec() and 2.2 supports
					// that but has an openURL() method as well that we
					// currently ignore.
					jvm = MRJ_2_1;
				} else if (version == 3.0) {
					jvm = MRJ_3_0;
	...
	private static Object locateBrowser() {
		...
		case MRJ_2_0:
			try {
				...
				// call locateBrowser(), we'll get the same AppleEvent, to which
				// we'll already have added the relevant parameter. Instead,
				// regenerate the AppleEvent every time.
		...
		case MRJ_2_1:
			...
			for (int i = 0; i < systemFolderFiles.length; i++) {
				try {
					...
					// and a type of 'FNDR'. Only requiring the type results in
					// non-Finder applications being picked up on certain Mac OS
					// 9 systems, especially German ones, and sending a GURL
					// event to those applications results in a logout under
					// Multiple Users.
	 ...
	 * @throws RuntimeException If the web browser could not be located or does not
	 *                          run
	 */
			...
			public void run() {
				try {
					...
					case MRJ_2_0:
						...
						break;
					case MRJ_2_1:
						Runtime.getRuntime().exec(new String[] { (String) browser, urlString });
						break;
					case MRJ_3_0:
					...
					case WINDOWS_9x:
						...
						Process process = Runtime.getRuntime()
								.exec(new String[] { (String) browser, FIRST_WINDOWS_PARAMETER,
										SECOND_WINDOWS_PARAMETER, THIRD_WINDOWS_PARAMETER, '"' + urlString + '"' });
						...
						break;
					case OTHER:
						// Assume that we're on Unix and that firefox is
						// installed
						process = Runtime.getRuntime().exec(new String[] { (String) browser, urlString });
						try {
							process.waitFor();
							process.exitValue();
						} catch (InterruptedException ie) {
	...
	public static void main(String[] args) throws Exception {
		BrowserLauncher.openURL(new URL("http://www.google.com"));
	}
}