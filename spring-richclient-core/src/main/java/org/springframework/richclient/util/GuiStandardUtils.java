...
public class GuiStandardUtils {
	 ...
	 * specific dialog.
	 * 
	 * @param dialogName the short name of the dialog.
	 */
	...
	/**
	 * Make a horizontal row of buttons of equal size, whch are equally spaced, and
	 * aligned on the right.
	 * 
	 * <P>
	 * The returned component has border spacing only on the top (of the size
	 * recommended by the Look and Feel Design Guidelines). All other spacing must
	 * be applied elsewhere ; usually, this will only mean that the dialog's
	 * top-level panel should use {@link #buildStandardBorder}.
	 * 
	...
	/**
	 * Make a vertical row of buttons of equal size, whch are equally spaced, and
	 * aligned on the right.
	 * 
	 * <P>
	 ...
	 * determined by the layout manager, which adjusts for locale-specific strings
	 * and customized fonts. (See this
	 * <a href="http://java.sun.com/products/jlf/ed2/samcode/prefere.html">Sun doc
	...
	/**
	 * An alternative to multi-line labels, for the presentation of several lines of
	 * text, and for which the line breaks are determined solely by the control.
	 * 
	 * @param text text that does not contain newline characters or html.
	 * @return <code>JTextArea</code> which is not editable, has improved spacing
	 *         over the supplied default (placing {@link UIConstants#ONE_SPACE}on
	 *         the left and right), and which wraps lines on word boundarie.
	 */
	...
	/**
	 * An alternative to multi-line labels, for the presentation of several lines of
	 * text, and for which line breaks are determined solely by <code>aText</code>,
	 * and not by the control.
	 * 
	 * @param text the text to be placed in the text area.
	 * @return <code>JTextArea</code> which is not editable and has improved spacing
	 *         over the supplied default (placing {@link UIConstants#ONE_SPACE}on
	 *         the left and right).
	 */
	...
	/**
	 * If aLabel has text which is longer than MAX_LABEL_LENGTH, then truncate the
	 * label text and place an ellipsis at the end; the original text is placed in a
	 * tooltip.
	 * 
	 * This is particularly useful for displaying file names, whose length can vary
	 * widely between deployments.
	 * 
	 * @param label The label to truncate if length() > MAX_LABEL_LENGTH.
	 */
	...
	/**
	 * Useful debug function to place a colored, line border around a component for
