...
public class CustomizableFocusTraversalPolicy extends LayoutFocusTraversalPolicy {
	...
	/**
	 * Installs an instance of CustomizableFocusTraversalPolicy as the default focus
	 * traversal policy.
	 */
	public static void installCustomizableFocusTraversalPolicy() {
		KeyboardFocusManager.getCurrentKeyboardFocusManager()
				.setDefaultFocusTraversalPolicy(new CustomizableFocusTraversalPolicy());
	}

	/**
	 * Sets a custom focus traversal order for the given container. Child components
	 * for which there is no order specified will receive focus after components
	 * that do have an order specified in the standard "layout" order.
	 * 
	 * @param container         the container
	 * @param componentsInOrder a list of child components in the order that thay
	...
	private static class CustomizableFocusTraversalComparator implements Comparator {
		...
		private Map getFocusOrder(Component comp) {
			...
			return (Map) ((parent instanceof JComponent)
					? ((JComponent) parent).getClientProperty(FOCUS_ORDER_PROPERTY_NAME)
					: null);
		}
