...
/**
 * A skeleton {@link TableModel} implementation that adds to the
 * {@link AbstractTableModel} class from the core Java API by providing the
 * functionality to manage the underlying collection of data for the table.
 *
 * @author Keith Donald
 */
public abstract class BaseTableModel extends AbstractTableModel implements MutableTableModel {
	...
	/**
	 * The collection of objects that represent the rows of the table. Class
	 * invariant; this must never be null.
	 */
	private List rows;
	...
	/**
	 * Overwrites the existing table rows with the given collection and fires an
	 * appropriate event to all registered listeners.
	 *
	 * @param rows The collection of rows that will overwrite the existing
	...
	/**
	 * Sets the flag that indicates whether or not row numbers are to appear in the
	 * first column of the displayed table.
	 *
	...
	/**
	 * Returns true if row numbers are to appear in the first column of the
	 * displayed table (default is true).
	 *
	 * @return The flag to show row numbers in the first column of the displayed
	 *         table.
	 */
	...
	/**
	 * Returns the number of columns, excluding the column that displays row numbers
	 * if present.
	 *
	 ...
	 * 
	 * @param columnIndex The zero-based index of the column whose type will be
	 *                    returned.
	 * 
	 * @throws IndexOutOfBoundsException if {@code columnIndex} is not within the
	 *                                   bounds of the column range for a row of the
	 *                                   table.
	 */
	@Override
	...
	/**
	 * Returns the name to be displayed in the header for the column at the given
	 * position.
	 * 
	 * @param columnIndex The zero-based index of the column whose name will be
	 *                    returned.
	 * 
	 * @throws IndexOutOfBoundsException if {@code columnInde} is not within the
	 *                                   bounds of the column range for a row of the
	 *                                   table.
	 */
	@Override
	...
	/**
	 * Returns the array of column headers other than the column displaying the row
	 * numbers, if present.
	 *
	...
	/**
	 * Subclasses must implement this method to return the value at the given column
	 * index for the given object.
	 *
	...
	/**
	 * Subclasses may override this method to determine if the cell at the specified
	 * row and column position can be edited. Default behaviour is to always return
	 * false.
	 *
	...
	/**
	 * Subclasses may implement this method to set the given value on the property
	 * at the given column index of the given row object. The default implementation
	 * is to do nothing.
	 *
	 ...
	 * 
	 * @throws IndexOutOfBoundsException if {@code rowIndex} is not within the
	 *                                   bounds of the collection of rows for this
	 *                                   table model.
	 */
	 ...
	 *
	 * @return The collection rows. The collection may be empty but will never be
	 *         null.
	 */
	public List getRows() {
		return rows;
	...
	/**
	 * Returns the collection of data from the given column of each row in the table
	 * model.
	 *
	 ...
	 *                                   bounds of the number of columns for the
	 *                                   rows in this table model.
	 */
	public List getColumnData(int column) {
		...
		if (column < 0 || column >= columnCount) {
			throw new IndexOutOfBoundsException(
					"The given column index [" + column + "] is outside the bounds of the number of columns ["
							+ columnCount + "] in the rows of this table model.");
		}
	...
	/**
	 * Returns the index of the first row containing the specified element, or -1 if
	 * this table model does not contain this element.
	 *
	 * @param obj The object whose row number will be returned.
	 * @return The index of the first row containing the given object, or -1 if the
	 *         table model does not contain the object.
	 */
	...
	public void remove(int firstIndex, int lastIndex) {
		...
		if (lastIndex < firstIndex) {
			throw new IllegalArgumentException(
					"lastIndex [" + lastIndex + "] cannot be less than firstIndex [" + firstIndex + "]");
		}

		if (firstIndex < 0 || firstIndex >= this.rows.size()) {
			throw new IndexOutOfBoundsException(
					"The specified starting index [" + firstIndex + "] is outside the bounds of the rows collection "
							+ "which only has [" + this.rows.size() + "] elements.");
		}

		if (lastIndex >= this.rows.size()) {
			throw new IndexOutOfBoundsException(
					"The specified end index [" + lastIndex + "] is outside the bounds of the rows collection "
							+ "which only has [" + this.rows.size() + "] elements.");
		}
	...
	public void remove(int[] indexes) {
		...
		if (indexes[0] < 0 || indexes[0] >= this.rows.size()) {
			throw new IndexOutOfBoundsException(
					"The specified index [" + indexes[0] + "] is outside the bounds of the rows collection "
							+ "which only has [" + this.rows.size() + "] elements.");
		}

		if ((indexes[indexes.length - 1]) >= this.rows.size()) {
			throw new IndexOutOfBoundsException("The specified end index [" + indexes[indexes.length - 1]
					+ "] is outside the bounds of the rows collection " + "which only has [" + this.rows.size()
					+ "] elements.");
		}
	...
	/**
	 * Returns the array of class types for the columns displayed by this table
	 * model.
	 *
	...
	/**
	 * Returns the array of column headers for the columns displayed by this table
	 * model.
	 *
	 * @return The array of column headers, never null.
	 */
	...
	/**
	 * Subclasses must implement this method to return the array of class types for
	 * the columns displayed by this table model.
	 *
	 * @return The array of column class types, never null.
	 */
	...
	/**
	 * Subclasses must implement this method to return the array of column headers
	 * for the columns to be displayed by this table model.
	 * 
	 * @return The array of column headers, never null.
	 */
