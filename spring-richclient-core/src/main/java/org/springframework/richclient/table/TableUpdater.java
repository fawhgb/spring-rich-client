...
public class TableUpdater extends Thread {
	...
	/**
	 * Publishes changes on the event dispatching thread when the system isn't busy.
	 * This blocks the caller until the changes have been published.
	 */
	private void publishChanges() {
		// And wait until there are no pending events.
		/*
		 * try { tableModel.lock(); } catch (InterruptedException e) {
		 * System.err.println("Table updater interrupted on table lock attempt. Nothing
		 * updated."); return; }
		 */
