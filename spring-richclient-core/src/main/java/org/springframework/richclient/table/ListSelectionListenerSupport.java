...
/**
 * Helper class used when listening to selection events on a <code>JTable</code>
 * or a <code>JList</code>.
 *
...
public class ListSelectionListenerSupport implements ListSelectionListener {
	 ...
	 * <code>ListSelectionEvent</code>. If the event is coming from a JList, the
	 * source of the event is the JList itself. If the event is coming from a
	 * JTable, the source of the event is the ListSelectionModel (talk about
	 * consistency)
	 *
	 * @param e the event
	...
	/**
	 * Called when multiple rows are selected. Override this method to handle
	 * multiple selection
	 *
