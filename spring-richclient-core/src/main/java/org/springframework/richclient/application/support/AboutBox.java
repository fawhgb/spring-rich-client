...
public class AboutBox {
			...
			if (aboutTextPath != null) {
				try {
					...
					String text = FileCopyUtils
							.copyToString(new BufferedReader(new InputStreamReader(aboutTextPath.getInputStream())));
	...
	protected static class HtmlScroller extends JViewport implements HyperlinkListener {
		 ...
		 *
		 * @param antiAlias   antialias the rendered HTML
		 * @param initalDelay inital delay after which scrolling begins
		 * @param speedPixSec scoll speed in pixels per second
		 * @param fps         number of updates per second
		 */
