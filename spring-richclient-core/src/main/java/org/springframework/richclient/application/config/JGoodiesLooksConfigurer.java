...
/**
 * <p>
 * JGoodies looks configurer bean. Allows to set various options available in
 * the JGoodies looks library. Use this as follows:
 * </p>
 * <p/>
 *
 * <pre>
 ...
 * <p/>
 * <p>
 * Additionally the LaF FQN can be set using {@link #setLaFName(String)}. Use
 * the static name constants defined in JGoodies
 * {@link com.jgoodies.looks.Options} to set this property. Default LaF if not
 * specified is the Options#PLASTICXP_NAME. It is possible to define other LaF's
 * as well, but any other property set in this bean will not be in effect if
 * used in that way. If you do need to set another LaF without additional
 * configuration use
 * {@link org.springframework.richclient.application.config.UIManagerConfigurer}
...
public class JGoodiesLooksConfigurer implements InitializingBean {
	...
	/**
	 * @param highContrastEnabled set to <code>true</code> if high contrast should
	 *                            be enabled.
	 ...
	 * <li>{@link com.jgoodies.looks.Options#DEFAULT_LOOK_NAME</li> </ul>
	 *
	 * <p>Default LaF if not specified is Options#PLASTICXP_NAME. Note that you
	 * could mention any LaF FQN here, but all other options would then be
	 * ignored.</p>
	 *
	 * @param laFName the FQN of the LaF you want to install on the
	 *                UIManagerConfigurer.
	 */
