 ...
 * {@link #SPLASH_SCREEN_BEAN_ID} and {@link #APPLICATION_BEAN_ID} respectively,
 * in one of the application contexts provided to the constructor.
 * </p>
 *
 * <p>
...
public class ApplicationLauncher {
	...
	/**
	 * Launches the application defined by the Spring application context file at
	 * the provided classpath-relative location.
	 *
	...
	/**
	 * Launches the application defined by the Spring application context files at
	 * the provided classpath-relative locations.
	 *
	...
	/**
	 * Launches the application defined by the Spring application context files at
	 * the provided classpath-relative locations. The application context file
	 * specified by {@code startupContext} is loaded first to allow for quick
	 * loading of the application splash screen. It is recommended that the startup
	 * context only contains the bean definition for the splash screen and any other
	 * beans that it depends upon. Any beans defined in the startup context will not
	 * be available to the main application once launched.
	 *
	 ...
	 * Launches the application defined by the Spring application context files at
	 * the provided classpath-relative locations. The application context file
	 * specified by {@code startupContextPath} is loaded first to allow for quick
	 * loading of the application splash screen. It is recommended that the startup
	 * context only contains the bean definition for the splash screen and any other
	 * beans that it depends upon. Any beans defined in the startup context will not
	 * be available to the main application once launched.
	 ...
	 *                                   startup context file. May be null or empty.
	 * @param rootContextConfigLocations The classpath-relative locations of the
	 *                                   main application context files.
	 *
	 * @throws IllegalArgumentException if {@code rootContextConfigLocations} is
	 *                                  null or empty.
	 */
	public ApplicationLauncher(String startupContextPath, String[] rootContextConfigLocations) {
		...
		org.springframework.util.Assert.notEmpty(rootContextConfigLocations,
	 ...
	 *
	 * @throws IllegalArgumentException if {@code rootApplicationContext} is null.
	 */
	...
	/**
	 * Launch the application using a startup context from the given location and a
	 * pre-loaded application context.
	 *
	 * @param startupContextPath     the classpath-relative location of the starup
	 *                               application context file. If null or empty, no
	...
	/**
	 * Returns an application context loaded from the bean definition file at the
	 * given classpath-relative location.
	 *
	 * @param startupContextPath The classpath-relative location of the application
	 *                           context file to be loaded. May be null or empty.
	 *
	...
	/**
	 * Returns an {@code ApplicationContext}, loaded from the bean definition files
	 * at the classpath-relative locations specified by {@code configLocations}.
	 *
	 * <p>
	 * If a splash screen has been created, the application context will be loaded
	 * with a bean post processor that will notify the splash screen's progress
	 * monitor as each bean is initialized.
	 * </p>
	 *
	...
	private ApplicationContext loadRootApplicationContext(String[] configLocations, MessageSource messageSource) {
		final ClassPathXmlApplicationContext applicationContext = new ClassPathXmlApplicationContext(configLocations,
				false);
	...
	private void setRootApplicationContext(ApplicationContext context) {
		org.springframework.util.Assert.notNull(context, "The root rich client application context is required");
		this.rootApplicationContext = context;
	}

	/**
	 * Launches the rich client application. If no startup context has so far been
	 * provided, the main application context will be searched for a splash screen
	 * to display. The main application context will then be searched for the
	 * {@link Application} to be launched, using the bean name
	...
	private void launchMyRichClient() {
		...
		try {
			// To avoid deadlocks when events fire during initialization of some swing
			// components
			// Possible to do: in theory not a single Swing component should be created
			// (=modified) in the launcher thread...
	...
	/**
	 * Searches the given bean factory for a {@link SplashScreen} defined with the
	 * bean name {@link #SPLASH_SCREEN_BEAN_ID} and displays it, if found.
	 *
	 * @param beanFactory The bean factory that is expected to contain the splash
	 *                    screen bean definition. Must not be null.
	 *
