...
public abstract class AbstractApplicationPage extends AbstractControlFactory implements ApplicationPage {
	...
	/**
	 * Called when the <code>PageComponent</code> changes any of its properties
	 * (display name, caption, icon, ...).
	 * <p>
	 * This method should be overridden when these changes must be reflected in the
	 * ui.
	 * 
	...
	protected void setActiveComponent() {
		if (pageComponents.size() > 0) {
			setActiveComponent(pageComponents.get(0));
		}
	}
	...
	public void setActiveComponent(PageComponent pageComponent) {
		...
		if (!pageComponents.contains(pageComponent) || this.activeComponent == pageComponent
	 ...
	 * This could involve making an internal frame visible, selecting a tab in a
	 * tabbed pane, ...
	 * 
	 * @param pageComponent the <code>PageComponent</code>
	 ...
	 * one).
	 * <p>
	 * Returns <code>false</code> if this <code>ApplicationPage</code> doesn't
	 * contain the given <code>PageComponent</code>.
	 * 
	 * @param pageComponent the <code>PageComponent</code>
	...
	public boolean close(PageComponent pageComponent) {
		if (!pageComponent.canClose() || !pageComponents.contains(pageComponent)) {
			return false;
		}
	...
	/**
	 * Adds the pageComponent to the components list while registering listeners and
	 * firing appropriate events. (not yet setting the component as the active one)
	 * 
	...
	public ViewDescriptorRegistry getViewDescriptorRegistry() {
		if (viewDescriptorRegistry == null) {
			viewDescriptorRegistry = (ViewDescriptorRegistry) ApplicationServicesLocator.services()
	...
	public PageComponentPaneFactory getPageComponentPaneFactory() {
		if (pageComponentPaneFactory == null) {
			pageComponentPaneFactory = (PageComponentPaneFactory) ApplicationServicesLocator.services()
					.getService(PageComponentPaneFactory.class);
		}
