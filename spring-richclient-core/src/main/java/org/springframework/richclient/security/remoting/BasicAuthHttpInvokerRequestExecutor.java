 ...
 *
 */
public class BasicAuthHttpInvokerRequestExecutor extends SimpleHttpInvokerRequestExecutor
	...
	/**
	 * Provided so subclasses can perform additional configuration if required (eg
	 * set additional request headers for non-security related information etc).
	 * 
	 ...
	 * authentication. Following that a call to {@link #doPrepareConnection} is made
	 * to allow subclasses to apply any additional configuration desired to the
	 * connection prior to invoking the request.
	 * <p>
	 * The previously saved authentication token is used to obtain the principal and
	 * credentials. If the saved token is null, then the "Authorization" header will
