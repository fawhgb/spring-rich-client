 ...
 * {@link org.springframework.security.AuthenticationManager} to be used to
 * handle authentication (login) requests. This would be done like this:
 *
 * <pre>
...
public class DefaultApplicationSecurityManager implements ApplicationSecurityManager, InitializingBean {
	 ...
	 * ApplicationServices. Since this bean won't be defined in the context under
	 * these circumstances, we need to perform some auto-configuration of our own.
	 * <p>
	 * Auto-configuration consists of trying to locate an AuthenticationManager (in
	 * one of several classes) in the application context. This auto-configuration
	 * is also attempted after the bean is constructed by the context if the
	 * authenticationManager property has not been set. See
	...
	/**
	 * Process a login attempt and fire all related events. If the authentication
	 * fails, then a {@link AuthenticationFailedEvent} is published and the
	 * exception is rethrown. If the authentication succeeds, then an
	 * {@link AuthenticationEvent} is published, followed by a {@link LoginEvent}.
	 * 
	...
	/**
	 * Ensure that we have an authentication manager to work with. If one has not
	 * been specifically wired in, then look for beans to "auto-wire" in. Look for a
	 * bean of one of the following types (in order): {@link ProviderManager},
