 ...
 * of BASIC authentication on each HTTP request. This factory takes care of
 * instantiating the proper invocation executor,
 * {@link BasicAuthHttpInvokerRequestExecutor}, and keeping it up to date with
 * the latest user credentials. Once a more complete AOP implementation is
 * available, then this "token forwarding" can be removed as the default
 * executor is already wired to receive notifications when it is constructed by
 ...
 * <p>
 * {@link AuthenticationAware} is implemented in order to get notifications of
 * changes in the user's credentials. Please see the class documentation for
...
public class BasicAuthHttpInvokerProxyFactoryBean extends HttpInvokerProxyFactoryBean implements AuthenticationAware {
	...
	/**
	 * Handle a change in the current authentication token. Pass it along to the
	 * executor if it's of the proper type.
	 *
