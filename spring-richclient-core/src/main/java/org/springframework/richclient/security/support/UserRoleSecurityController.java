 ...
 * {@link org.springframework.richclient.core.Authorizable}, according to the
 * roles held by the currently authenticated user. If the current user holds one
 * or more of the configured roles, then the associated objects are authorized.
 * Otherwise, they are not authorized.
 * <p>
 * The roles on which to authorize the controlled objects are specified via the
 * <code>roles</code> property. It should be a comma-separated list of role
 * names.
 * <p>
 * No secured object is used in preparing the ConfigAttributeDefinition. This
 * means that the configuration is not specific to a given object (no per-object
 * ACL's). The access decision is made strictly on the roles held by the user.
 * <p>
 * The default access decision manager is an {@link AffirmativeBased} instance
 * using a plain {@link RoleVoter}. You can override this by setting the
...
public class UserRoleSecurityController extends AbstractSecurityController {
	 ...
	 * secured objects will be authorized if the user holds one or more of these
	 * roles. This should be specified as a simple list of comma separated role
	 * names.
	 *
	 * @param roles
	 */
	...
	/**
	 * Get the secured object on which we are making the authorization decision. We
	 * return null as no specific object is to be considered in the decision.
	 *
	 ...
	 *                      is to be retrieved. This may be null.
