...
/**
 * Default implementation of the SecurityControllerManager. The controller map
 * can be set during bean initialization. Also, if the map is not set, or does
 * not contain an entry for a requested security controller Id, then an attempt
 * to retrieve a bean instance with the given id will be made. Thus, no map
 * entries are required for security controllers that are referenced by their
 * application context bean id.
 * <p>
 * This implementation also provides for a <em>fallback</em> security
 * controller. The fallback controller is registered with the
 * {@link #setFallbackSecurityController(SecurityController)} method. Once
 * registered, any call to {@link #getSecurityController(String)} that would
 * have normally resulted in not finding a controller will instead return the
 * fallback controller.
 * <p>
...
public class DefaultSecurityControllerManager implements SecurityControllerManager {
	 ...
	 * setSecurityControllerMap(java.util.Map)
	 */
	@Override
	...
	/**
	 * Get the security controller for the given id. If the id is registered in our
	 * map, then return the registered controller. If not, then try to obtain a bean
	 * with the given id if it implements the SecurityController interface.
	 *
	...
	/**
	 * Set the fallback security controller. This controller will be returned for
	 * any requested controller Id that is not found in the registry.
	 *
