 ...
 * <P>
 * Presents a dialog to the user to collect login credentials. It then invokes
 * the {@link ApplicationSecurityManager#doLogin} method to validate the
 * credentials. The ApplicationSecurityManager is responsible for updating the
 * security context and firing appropriate security events.
 * <p>
 * The default user name can be specified as a configuration parameter. This is
 * useful when combined with
 * {@link org.springframework.beans.factory.config.PropertyPlaceholderConfigurer}
 * to install the current (system) username.
 * <p>
 * If the login is unsuccesful, a message is presented to the user and they are
 * offered another chance to login.
 * <p>
 * The <code>closeOnCancel</code> property controls what happens if the user
 * cancels the login dialog. If closeOnCancel is true (the default), if there is
 * no valid authentication in place (from a previous login) then the application
 * is closed. If it is false or an authentication token is available, then no
 * action is taken other than closing the dialog.
 * <p>
 * The <code>clearPasswordOnFailure</code> controls the handling of the password
 * field after a login failure. If clearPasswordOnFailure is <code>true</code>
 * (the default), then the password field will be cleared after the failure is
 * reported.
 * <p>
...
public class LoginCommand extends ApplicationWindowAwareCommand {
	 ...
	 *
	 * @param displaySuccessMessage displays an information message upon successful
