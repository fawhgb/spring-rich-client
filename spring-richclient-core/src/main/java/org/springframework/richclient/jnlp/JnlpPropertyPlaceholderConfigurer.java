 ...
 * <p/>
 * <p>
 * If a placeholder could not be resolved against the provided local properties
 * within the application, this configurer will fall back to the JNLP
 * properties. Can also be configured to let jnlp properties override local
 * properties (contextOverride=true).
 * <p/>
 * <p>
 * If not running within a JNLP context (or any other context that is able to
 * satisfy the BasicService.lookup call), this class will use the
 * fallBackWebAppContextUrl. This allows for keeping
...
public class JnlpPropertyPlaceholderConfigurer extends PropertyPlaceholderConfigurer {
	...
	/**
	 * Set whether the JNLP properties should override local properties within the
	 * application. Default is "false": JNLP properties settings serve as fallback.
	 * <p>
	 * Note that system properties will still override JNLP properties, if the
	 * system properties mode is set to "SYSTEM_PROPERTIES_MODE_OVERRIDE".
	 *
	 * @see #setSystemPropertiesModeName
	 ...
	 * This is usefull for testing outside JNLP (Java webstart). This defaults to
	 * <code>http://localhost:8080/</code>. Ussually you 'll want to set this to
	 * <code>http://localhost:8080/mywebapp/</code>.
	 *
	 * @param fallBackWebAppContextUrl the webAppContextUrl to fall back on ending
	 *                                 with a slash
	 */
	 ...
	 * WebAppContext. Default this is <code>/</code>, which means that the JNLP file
	 * is in the root of your webapp. If your JNLP file isn't in the root of you
	 * webapp, change it. For example for
	 * <code>http://localhost:8080/mywebapp/dist/jnlp/myswingapp.jnlp</code> you
	 * would set this to <code>/dist/jnlp/</code>.
	 *
	 * @param jnlpRelativeDirectoryPathFromWebAppContext the relative directory path
	 ...
	 * webstart, <code>fallBackWebAppContextUrl</code> is returned. For example this
	 * could return <code>http://localhost:8080/mywebapp/</code>.
	 *
