...
/**
 * A purger that looks through to a throwable chain and can select one to
 * unwrap.
 *
...
public class DefaultExceptionPurger implements ExceptionPurger {
	 ...
	 *
	 * @param includeThrowableClass used as a singleton list for
	...
	/**
	 * Sets Throwables that if found, are unwrapped. These Throwables are ussually
	 * very specific exceptions, for example: LoginCredentialsExpiredException. The
	 * earliest throwable found is selected.
	 * </p>
	 * Given a chain A1->B1->C1->B2->D1: {A} returns A1; {B} returns B1; {D} returns
	 * D1; {Z) returns A1; {C, Z} returns C1; {B, D} returns B1; {D, B} return B1;
	 * </p>
	 * When combined, includeThrowableClassList takes priority over
	 ...
	 *
	 * @param excludeThrowableClass used as a singleton list for
	 *                              excludeThrowableClassList
	 */
	...
	/**
	 * Sets Throwables that if found, its cause is unwrapped. These Throwables are
	 * ussually very general wrapper exceptions, for example: WrapperException. The
	 * last throwable found's cause is selected. If the cause is null, itself is
	 * selected.
	 * </p>
	 * Given a chain A1->B1->C1->B2->D1: {A} returns B1; {B} returns D1; {D} returns
	 * D1; {Z) returns A1; {C, Z} returns B2; {C, D} returns D1; {D, C} return D1;
	 * </p>
	 * When combined, includeThrowableClassList takes priority over
	 * excludeThrowableClassList.
	 *
