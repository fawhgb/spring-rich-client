...
public abstract class AbstractDialogExceptionHandler extends AbstractLoggingExceptionHandler
	...
	/**
	 * Where or not the shown dialog should be modal (see JDialog API). The default
	 * is true;
	 *
	 * @param modalDialog
	 */
	...
	/**
	 * Wheter or not the user should be asked or obligated to shutdown the
	 * application. The default is ASK.
	 *
	...
	public void notifyUserAboutException(Thread thread, Throwable throwable) {
		Object[] options;
		// TODO implement ability to deal with mnemonics (for example "&Shutdown" in
		// messages.properties)
		switch (shutdownPolicy) {
		case NONE:
			options = new String[] { messageSourceAccessor.getMessage(DIALOG_EXCEPTION_HANDLER_KEY + ".none.ok") };
			break;
		case ASK:
			options = new String[] { messageSourceAccessor.getMessage(DIALOG_EXCEPTION_HANDLER_KEY + ".ask.shutdown"),
		...
		int result = JOptionPane.showOptionDialog(resolveParentFrame(), createExceptionContent(throwable),
				resolveExceptionCaption(throwable), JOptionPane.DEFAULT_OPTION, resolveMessageType(), null, options,
				options[0]);
		if ((shutdownPolicy == ShutdownPolicy.ASK && result == 0) || shutdownPolicy == ShutdownPolicy.OBLIGATE) {
