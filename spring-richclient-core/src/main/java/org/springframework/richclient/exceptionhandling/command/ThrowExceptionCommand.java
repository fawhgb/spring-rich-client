package org.springframework.richclient.exceptionhandling.command;
...
/**
 * A development command used to test the exception handler configuration. Pops
 * up a dialog and allows the user to type in the full classname of an exception
 * to throw.
 * <p/>
 * It is usually not used in production and therefor configured with this
 * property: &lt;property name="visible" value="${development.mode}" /&gt;
 *
 * @author gds
 */
public class ThrowExceptionCommand extends ApplicationWindowAwareCommand {
	...
	private List<Class<? extends Throwable>> askUserThrowableClassList() {
		String throwableClassListString = (String) JOptionPane.showInputDialog(resolveParentFrame(),
				...
				"Which exception do you want to throw?", JOptionPane.INFORMATION_MESSAGE, null, null,
		...
		for (String throwableClassListStringToken : throwableClassListStringTokens) {
			...
			try {
				...
				throw new IllegalArgumentException(
						"ThrowExceptionCommand failure: Class (" + throwableClassListStringToken + ") does not exist.",
						e);
			}
	...
	private void throwThrowable(List<Class<? extends Throwable>> throwableClassList) {
		...
		if (throwable instanceof RuntimeException) {
			...
			throw new IllegalArgumentException("ThrowExceptionCommand failure: The root throwable (" + throwableClass
					+ ") should extend RuntimeException or Error.");
		}
	}
	...
	private Throwable buildThrowable(List<Class<? extends Throwable>> throwableClassList,
		...
		if (throwableClassList.isEmpty()) {
			try {
				throwable = throwableClass.getDeclaredConstructor(String.class).newInstance(THROWABLE_MESSAGE);
			} catch (NoSuchMethodException e) {
