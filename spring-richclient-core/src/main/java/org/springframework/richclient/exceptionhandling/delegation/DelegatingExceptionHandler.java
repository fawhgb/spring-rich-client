 ...
 * list based on the thrown exception and delegates the handling of the
 * exception to it.
 * <p/>
 * This class works very similar to catch statements: the first delegate which
 * can handle the exception will handle it. For example, consider 3 simple
 * delegates for the following classes in this order: NullPointerException (1),
 * RuntimeException (2), IllegalArgumentException (3). A thrown
 * IllegalArgumentException will be handled by the (2) handler. The (3) handler
 * is useless.
 *
...
public class DelegatingExceptionHandler extends AbstractRegisterableExceptionHandler implements InitializingBean {
	...
	/**
	 * Sets the list of delegates. This is not a map because the order is important
