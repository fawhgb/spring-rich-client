...
/**
 * Displays a message to the user which is fetched from the I18N files based on
 * the class and superclasses of the throwable.
 * <p/>
 * For example if an IllegalArgumentException is thrown, it will search for
 * java.lang.IllegalArgumentException.caption and
 * java.lang.IllegalArgumentException.description first, and if it cant find
 * that it will try in order: java.lang.RuntimeException.caption/description,
 ...
 * first. Note for the repacing of {0} to work in a property file double
 * quotes(") need to be escaped (\") and that single quotes (') should be
 * avoided (escaping doesn't seem to work).
 *
...
public class MessagesDialogExceptionHandler extends AbstractDialogExceptionHandler {
	...
	/**
	 * If messagesKey is set, the caption and description shown in the dialog are
	 * not based dynamically on the throwable, but instead statically on the keys
	 * messageKey.caption and messageKey.description.
	 * 
	...
	public String resolveExceptionCaption(Throwable throwable) {
		...
		return messageSourceAccessor.getMessage(new DefaultMessageSourceResolvable(messagesKeys, messagesKeys[0]));
	}
	...
	protected String[] getMessagesKeys(Throwable throwable, String keySuffix) {
		...
		if (throwable instanceof SQLException) {
			messageKeyList
					.add(SQLException.class.getName() + "." + ((SQLException) throwable).getErrorCode() + keySuffix);
		}
