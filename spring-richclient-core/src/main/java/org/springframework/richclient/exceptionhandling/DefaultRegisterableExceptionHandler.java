...
/**
 * @TODO extends AbstractRegisterableExceptionHandler as soon as
 *       spring-richclient is minimum 1.5
...
public class DefaultRegisterableExceptionHandler implements RegisterableExceptionHandler {
	 ...
	 *
	 * @TODO remove as soon as this class extends
	...
	public void uncaughtException(Thread thread, Throwable throwable) {
		...
		if (throwable instanceof MessageSourceResolvable) {
			exceptionMessage = ((MessageSourceAccessor) ApplicationServicesLocator.services()
					.getService(MessageSourceAccessor.class)).getMessage((MessageSourceResolvable) throwable);
		} else {
		...
		if (!StringUtils.hasText(exceptionMessage)) {
			...
			exceptionMessage = ((MessageSourceAccessor) ApplicationServicesLocator.services()
					.getService(MessageSourceAccessor.class)).getMessage("applicationDialog.defaultException",
							defaultMessage);
		}
