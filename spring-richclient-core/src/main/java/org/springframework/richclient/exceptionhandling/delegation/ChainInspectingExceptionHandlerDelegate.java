...
/**
 * Checks the exception chain to determine if it wants to handle it. In most
 * cases this class is overkill and SimpleExceptionHandlerDelegate with a purger
 * will suffice. Think about it: just purging out MySpecificLoginException
 * suffices most of the time, it doesn't really matter that it's wrapped in
 * MyWrapperException.
 *
...
//  }
	...
	public Throwable findChainPartThrowable(ChainPart chainPart, Throwable firstThrowable) {
		...
		while (!chainPart.getThrowableClass().isInstance(e) || (relativeDepth < minimumRelativeDepth)) {
			relativeDepth++;
			...
			if ((maximumRelativeDepth >= 0) && (relativeDepth > maximumRelativeDepth)) {
