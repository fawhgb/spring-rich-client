...
/**
 * Uncaught exception handler designed to work with JDK 1.4 and 1.5's primitive
 * API for registering exception handlers for the event thread.
 *
 * It's impossible to set an exception handler for the event thread in jdk 1.4
 * (and 1.5). See http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4714232 So
 * this effectively only works in Sun's JDK.
 *
...
public class AwtExceptionHandlerAdapterHack {
	 ...
	 * dependencies into it, except by a static reference like this.
	 */
	private static RegisterableExceptionHandler exceptionHandlerDelegate = null;

	/**
	 * Sets the {@link #SUN_AWT_EXCEPTION_HANDLER_KEY} system property to register
	 * this class as the event thread's exception handler. When called back, this
	 * class simply forwards to the delegate.
	 *
	 * @param exceptionHandlerDelegate the "real" exception handler to delegate to
	...
	public static void registerExceptionHandler(RegisterableExceptionHandler exceptionHandlerDelegate) {
		...
		// Always sets even if previously set.
