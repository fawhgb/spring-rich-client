...
/**
 * Superclass of logging exception handlers. It handles a throwable by logging
 * it and notify it to the user. Subclasses determine how it's notified to the
 * user.
 *
...
public abstract class AbstractLoggingExceptionHandler extends AbstractRegisterableExceptionHandler {
	 ...
	 * <ul>
	 * <li>Case 1: The ExceptionHandler throws an exception, which is handled by the
	 * ExceptionHandler, which throws an exception, ...</li>
	 * <li>Case 2: notifyUserAboutException uses a modal dialog that triggers the
	 * event queue to do a focus lost That focus lost throws a new exception,
	 * catched by the event queue, which delegates it to the ExceptionHandler, which
	 * still hasn't returned from handling the original exception! Then the
	 * ExceptionHandler uses a modal dialog again, focus lost, throws exception, ...
	 * This causes a StackOverflowError or a hang-application-by-starvation due to
	 * too many dialogs.</li>
	 * </ul>
	 */
	@Override
	public final void uncaughtException(Thread thread, Throwable throwable) {
		...
		if (infiniteLoopDetected != null && infiniteLoopDetected.booleanValue()) {
			...
			// to report the first exception and lose performance due to an infinite event
			// loop
