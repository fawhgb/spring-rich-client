...
/**
 * An immutable parameter object consisting of the text, mnemonic character and
 * mnemonic character index that may be associated with a labeled component.
 * This class also acts as a factory for creating instances of itself based on a
 * string descriptor that adheres to some simple syntax rules as described in
 * the javadoc for the {@link #valueOf(String)} method.
 *
 * <p>
 * The syntax used for the label info descriptor is just the text to be
 * displayed by the label with an ampersand (&) optionally inserted before the
 * character that is to be used as a mnemonic for the label.
 * </p>
 *
 * <p>
 * Example: To create a label with the text {@code My Label} and the capital L
 * as a mnemonic, use the following descriptor:
 * </p>
 *
 * <pre>
 *     <code>My &Label</code>
 ...
 *
 * <p>
 * A backslash character (\) can be used to escape ampersand characters that are
 * to be displayed as part of the label's text. For example:
 * </p>
 *
 * <pre>
 *     <code>Save \& Run</code>
 ...
 *
 * <p>
 * Only one non-escaped backslash can appear in the label descriptor. Attempting
 * to specify more than one mnemonic character will result in an exception being
 * thrown. TODO finish comment regarding backslash chars in props file Note that
 * for label descriptors provided in properties files, an extra backslash will
 * be required to avoid the single backslash being interpreted as a special
 * character.
 * </p>
 *
 * @author Keith Donald
...
public final class LabelInfo {
	...
	/**
	 * Creates a new {@code LabelInfo} instance by parsing the given label
	 * descriptor to determine the label's text and mnemonic character. The syntax
	 * rules for the descriptor are as follows:
	 *
	 * <ul>
	 * <li>The descriptor may be null or an empty string, in which case, an instance
	 * with no text or mnemonic will be returned.</li>
	 * <li>The mnemonic character is indicated by a preceding ampersand (&).</li>
	 * <li>A backslash character (\) can be used to escape ampersand characters that
	 * are to be displayed as part of the label's text.</li>
	 * <li>A double backslash (a backslash escaped by a backslash) indicates that a
	 * single backslash is to appear in the label's text.</li>
	 ...
	 * </ul>
	 *
	 * @param labelDescriptor The label descriptor. The text may be null or empty,
	 *                        in which case a blank {@code LabelInfo} instance will
	 *                        be returned.
	 *
	 * @return A {@code LabelInfo} instance that is described by the given
	 *         descriptor. Never returns null.
	 *
	 * @throws IllegalArgumentException if {@code labelDescriptor} violates any of
	 *                                  the syntax rules described above.
	 */
	public static LabelInfo valueOf(final String labelDescriptor) {
		...
		// mnemonics work with VK_XXX (see KeyEvent) and only uppercase letters are used
		// as event
	...
	/**
	 * Confirms that the character at the specified index within the given label
	 * descriptor is a valid 'escapable' character. i.e. either an ampersand or
	 * backslash.
	 *
	 * @param index           The position within the label descriptor of the
	 *                        character to be checked.
	 * @param labelDescriptor The label descriptor.
	 *
	 * @throws NullPointerException     if {@code labelDescriptor} is null.
	 * @throws IllegalArgumentException if the given {@code index} position is
	 *                                  beyond the length of the string or if the
	 *                                  character at that position is not an
	 *                                  ampersand or backslash.
	 */
	private static void checkForValidEscapedCharacter(int index, String labelDescriptor) {

		if (index >= labelDescriptor.length()) {
			throw new IllegalArgumentException("The label descriptor contains an invalid escape sequence. Backslash "
		...
		if (escapedChar != '&' && escapedChar != '\\') {
			throw new IllegalArgumentException(
					"The label descriptor [" + labelDescriptor + "] contains an invalid escape sequence. Backslash "
							+ "characters (\\) must be followed by either an ampersand (&) or another " + "backslash.");
		}
	...
	/**
	 * Confirms that the given character is allowed to be used as a mnemonic.
	 * Currently, only spaces are disallowed.
	 *
	...
	/**
	 * Creates a new {@code LabelInfo} with the given text and no specified
	 * mnemonic.
	 *
	 * @param text The text to be displayed by the label. This may be an empty
	 *             string but cannot be null.
	 *
	 ...
	 *
	 * @throws IllegalArgumentException if {@code text} is null or if
	 *                                  {@code mnemonic} is a negative value.
	 */
	...
	/**
	 * Creates a new {@code LabelInfo} with the given text, mnemonic character and
	 * mnemonic index.
	 *
	 * @param text          The text to be displayed by the label. This may be an
	 *                      empty string but cannot be null.
	 * @param mnemonic      The character from the label text that acts as a
	 *                      mnemonic.
	 * @param mnemonicIndex The zero-based index of the mnemonic character within
	 *                      the label text. If the specified label text is an empty
	 *                      string, this property will be ignored and set to -1.
	 *
	 * @throws IllegalArgumentException if {@code text} is null, if {@code mnemonic}
	 *                                  is a negative value, if
	 *                                  {@code mnemonicIndex} is less than -1 or if
	 *                                  {@code mnemonicIndex} is outside the length
	 *                                  of {@code text}.
	 */
	public LabelInfo(String text, int mnemonic, int mnemonicIndex) {

		Assert.required(text, "text");
		org.springframework.util.Assert.isTrue(mnemonic >= 0, "mnemonic must be greater than or equal to 0");
		org.springframework.util.Assert.isTrue(mnemonicIndex >= -1,
				"mnemonicIndex must be greater than or equal to -1");

		org.springframework.util.Assert.isTrue(mnemonicIndex < text.length(),
	...
	/**
	 * Configures the given label with the property values described by this
	 * instance and then sets it as the label for the given component.
	 *
	...
	/**
	 * Configures the given button with the properties held in this instance. Note
	 * that this instance doesn't hold any keystroke accelerator information.
	 *
	...
	public void configureButton(AbstractButton button) {
		org.springframework.util.Assert.notNull(button);
	...
	/**
	 * Returns the character that is to be treated as the mnemonic character for the
	 * label.
	 *
	 ...
	 *
	 * @return The index of the mnemonic character, or -1 if no mnemonic index is
	 *         specified.
	 */
