...
/**
 * Creates a bridge between a <code>ValueModel</code> and an
 * <code>EventList</code>. This means that when the <code>ValueModel</code>
 * value changes, it's copied into the EventList. There's also a
 * {@link #synchronize()} method that manually copies the values.
 *
...
public class ValueModel2EventListBridge {
	...
	/**
	 * Same as calling
	 * {@link #ValueModel2EventListBridge(ValueModel, EventList, boolean)} with
	 * <code>manualSynchronize = false</code>.
	 * 
	 * @param valueModel the ValueModel, cannot be <code>null</code>
	 * @param eventList  the EventList, cannot be <code>null</code>
	...
	/**
	 * Creates a <code>ValueModel2EventListBridge</code> for the given ValueModel
	 * and EventList.
	 * 
	 * @param valueModel        the ValueModel, cannot be <code>null</code>
	 * @param eventList         the EventList, cannot be <code>null</code>
	 * @param manualSynchronize manual or automatic synchronize?
	 */
	...
	/**
	 * Synchronizes the EventList with the ValueModel. The values of the collection
	 * in the ValueModel are copied to the EventList. If the value in the ValueModel
	 * is <code>null</code>, the EventList will be emptied.
	 */
	public void synchronize() {
		System.out.println(Thread.currentThread().getName());
		eventList.clear();

		// call ValueModel.getValue() only once, as it's possible that getValue is
		// expensive (as can be the case with a
