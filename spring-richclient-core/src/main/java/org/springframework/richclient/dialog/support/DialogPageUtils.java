...
/**
 * Useful methods for working with {@link DialogPage}s. These methods make it
 * easier to place dialog pages in arbitrary containers, wire in message
 * listeners, wire actions to the page complete status, etc.
 * <p>
 * These helpers can be used to quickly construct a standard layout (like that
 * found in the {@link TitledPageApplicationDialog}, or you can use these
 * methods to construct the various components (title area and button bar) and
 * then lay things out as you like.
 * <p>
 * The standard layout is obtained using the
 * {@link #createStandardView(DialogPage, ActionCommand, ActionCommand)} or
 * {@link #createStandardView(DialogPage, Object[])} methods. To create the
 * pieces and do the layout independently, use
 * {@link #createTitlePane(DialogPage)} to get the title pane and wire your OK
 * command into the page complete status using
 * {@link #adaptPageCompletetoGuarded(DialogPage, Guarded)}. If you don't want
 * to use the standard title pane, you can easily arrange for the messages
 * coming from the dialog page to be sent to a {@link Messagable} of your choice
 * using {@link #addMessageMonitor(DialogPage, Messagable)}.
 *
...
public class DialogPageUtils {
	...
	/**
	 * Create a standard {@link TitlePane} wired to receive messages from the given
	 * dialog page. The title pane will also be configured from the dialog page's
	 * title and icon.
	 * 
	...
	/**
	 * Construct a complete standard layout for a dialog page. This is a panel with
	 * the title/message area at the top, the dialog page control in the center, and
	 * the command button bar (using the provided ok and cancel commands) on the
	 * bottom. The finishCommand provided will automatically be wired into the page
	 * complete status of the dialog page.
	 * 
	 * @param dialogPage    to process
	 ...
	 * Construct a complete standard layout for a dialog page. This is a panel with
	 * the title/message area at the top, the dialog page control in the center, and
	 * the command button bar (using the provided group of commands) on the bottom.
	 * You should have already wired any commands to the page complete status as
	 * needed.
	 * 
	 * @param dialogPage          to process
	...
	/**
	 * Create an adapter that will monitor the page complete status of the dialog
	 * page and adapt it to operations on the provided Guarded object. If the page
	 * is complete, then the guarded object will be enabled. If this page is not
	 * complete, then the guarded object will be disabled.
	 * 
	 * @param dialogPage to monitor
	...
	/**
	 * Internal class to handle the MESSAGE_PROPERTY property changes in a dialog
	 * page.
	 */
