...
/**
 * <code>FormComponentInterceptorFactory</code> implementation that can include
 * or exclude form models.
 *
 * @author Peter De Bruycker
 */
public abstract class ConfigurableFormComponentInterceptorFactory
		implements FormComponentInterceptorFactory, InitializingBean {
	 ...
	 * <code>null</code>. <br>
	 * If the included ids are specified and don't contain the given form model,
	...
	public final FormComponentInterceptor getInterceptor(FormModel formModel) {
		...
		if ((excludedFormModelIds != null && Arrays.asList(excludedFormModelIds).contains(formModel.getId()))
				|| (includedFormModelIds != null && !Arrays.asList(includedFormModelIds).contains(formModel.getId()))) {
			return null;
		}
