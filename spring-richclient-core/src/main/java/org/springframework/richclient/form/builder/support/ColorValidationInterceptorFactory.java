...
public class ColorValidationInterceptorFactory implements FormComponentInterceptorFactory {
	...
	/**
	 * The colors used with each component are set by a ComponentUI class. When the
	 * state of a component changes, the ComponentUI class will react on this and
	 * set the appropriate colors <b>if and only if</b> the current color on the
	 * component is implementing ResourceUI. (~ was created by the ComponentUI
	 * itself)
	 *
	 * That's why we have to remember this color instead of defining our own
	 * "normal" color which wouldn't be replaced by the inactive/active color scheme
	 * used with editable/enabled.
	 *
	 * The only thing to check actually is when the enable/editable states are
	 * changed (should be after this colorChanger).
	 *
	 * @see javax.swing.plaf.basic.BasicTextFieldUI#update(java.awt.Graphics,
	 *      JComponent)
	 *
	 * @author jh
	 *
	...
	private class ColorChanger implements Messagable {
		 ...
		 * Constructor.
		 *
		 * @param component component on which to change color.
		 */
		...
		/**
		 * Set the color according to the enabled state. When enabled, check component's
		 * enabled and editable state for correct background color.
		 */
		@Override
