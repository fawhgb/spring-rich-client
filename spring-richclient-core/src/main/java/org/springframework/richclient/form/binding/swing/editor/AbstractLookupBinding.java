...
public abstract class AbstractLookupBinding extends CustomBinding {
	...
	/**
	 * Parameter used to pass to the dataEditorCommand in order to skip
	 * initialization of the dataEditor.
	 */
	...
	/**
	 * Pop-up if no unique match is found. This is considered to be the sensible
	 * default.
	 */
	...
	/**
	 * Enable the command that is used to switch to the dataEditor view of this
	 * referable.
	 */
	...
	public AbstractLookupBinding(DefaultDataEditorWidget dataEditor, FormModel formModel, String formPropertyPath) {
		...
		// this will prevent initialize from being called twice: when losing focus
		// (searching for
	...
	/**
	 * Returns the parameter map that is passed to the dataEditor command. This
	 * allows for eg turning initialization of the dataEditor on/off.
	 */
	...
	/**
	 * Returns the id for the TitledApplicationDialog that shows up when pressing
	 * the button.
	 */
	...
	/**
	 * Set the id for the TitledApplicationDialog that shows up when pressing the
	 * button.
	 */
	...
	/**
	 * Return <code>true</code> if the value should be reverted when focus is lost
	...
	/**
	 * Set to <code>true</code> if the value should be reverted when focus is lost
	 * and no value is selected.
	 */
	...
	protected JComponent doBindControl() {
		...
		if (isEnableViewCommand()) {
			layout = new FormLayout(new ColumnSpec[] {
					...
					FormFactory.RELATED_GAP_COLSPEC, new ColumnSpec(ColumnSpec.LEFT, Sizes.MINIMUM, FormSpec.NO_GROW),
					FormFactory.RELATED_GAP_COLSPEC, new ColumnSpec(ColumnSpec.LEFT, Sizes.MINIMUM, FormSpec.NO_GROW) },
	 ...
	 *
	 * @deprecated use {@link #getKeyComponent()} instead or to access the text
	 *             directly, use {@link #getKeyComponentText()} and
	 *             {@link #setKeyComponentText(String)}.
	 */
	@Deprecated
	 ...
	 *
	 * @deprecated move implementation to {@link #createKeyComponent()} when
	 *             removing this method.
	 */
	@Deprecated
	protected JTextField createTextComponent() {
		JTextField textField = new JTextField();
		// Focustraversal keys moeten afgezet worden, anders wordt de keylistener niet
		// getriggered.
		textField.setFocusTraversalKeys(KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS, new HashSet<AWTKeyStroke>());
	...
	/**
	 * Return the text that is shown on the keyComponent and that should be used to
	 * lookup the referable. Most commonly this string contains the label of the
	 * referable.
	 */
	...
	/**
	 * Set the text of the referable label on the key component (normally the
	 * textField).
	 */
			...
			public void onTabKey(Component component) {
				...
				if (!empty && ((ref == null) || !textFieldValue.equals(getObjectLabel(ref)))) {
					...
					// exact match
						...
						// in dit geval krijg je een object uit de lijst terug, dit is niet
						// gedetaileerd,
				...
				// nothing filled in, underlying value isn't empty and we should not revert, set
				// null
	...
	/**
	 * Create a focus listener to attach to the textComponent and dataEditorButton
	 * that will decide what happens with the changed value. Here a revert can be
	 * done if no value is selected or a new value can be created as needed.
	 *
	...
	/**
	 * Initialize the dataEditor by passing the search referable as search
	 * parameter.
	 *
	 * @return a single object if the search has an unique match, a list if multiple
	 *         matches occurred or <code>null</code> if nothing was found.
	 */
	...
	/**
	 * Create an empty referable that is used to pass onto the dataEditor search
	 * method and that is used to set onto the valueModel if this binding is set to
	 * not revert upon yielding a <code>null</code> search result.
	 *
	 * @param textFieldValue the value of the textComponent.
	 * @return a Referable that represents the state of this binding when no real
	 *         object is available.
	 */
	...
	private class ReferableDataEditorViewCommand extends ActionCommand {
		...
		@Override
		protected void doExecuteCommand() {
			org.springframework.util.Assert.notNull(dataEditorViewCommandId);
	...
	/**
	 * Enable the viewCommand that switches the view to the dataEditor of this
	 * referable.
	 */
	...
	/**
	 * Returns <code>true</code> if the viewCommand should be shown. Default value
	 * is <code>false</code>.
	 */
	...
	/**
	 * Helper class to build a {@link java.awt.event.KeyListener} that reacts on
	 * tabs. Implement the {@link #onShiftTabKey(Component)} and/or
	 * {@link #onTabKey(Component)} methods as needed.
	 *
	 * @author Jan Hoskens
	 */
	protected static class TabKeyListener extends KeyAdapter {
		...
		/**
		 * Action to do when shift-tab is pressed. Default behavior is to transfer
		 * focus.
		 *
