...
public abstract class AbstractBinderSelectionStrategy
	...
	public Binder selectBinder(FormModel formModel, String propertyName) {
		...
		throw new UnsupportedOperationException(
				"Unable to select a binder for form model [" + formModel + "] property [" + propertyName + "]");
	}
	...
	/**
	 * Try to find a binder for the provided parentObjectType and propertyName. If
	 * no direct match found try to find binder for any superclass of the provided
	...
	/**
	 * Add a list of binders that are bound to propertyNames. Each element in the
	 * list should be a Properties element describing the binder and propertyName.
	 * For more information about the structure of the properties see
	 * {@link #setBinderForPropertyName(Properties)}.
	 *
	 * <br>
	 * <br>
	 * &lt;list&gt;<br>
	...
	/**
	 * Create/link a <code>Binder</code> to a propertyName from the given
	 ...
	 * <li><b>binder</b>: The Fully Qualified ClassName that will be used to
	 * instantiate the <code>Binder</code>.
	 * <li><b>binderRef</b>: The beanId that identifies the <code>Binder</code>
	 * which is defined elsewhere.
	 * </ul>
	 * <p>
	 * The first two keys are mandatory in combination with one of the two latter
	 * (binder or binderRef) The following two cases can be used to define a
	 * binder/propertyName combination: <br>
	 * <br>
	 * &lt;props&gt;<br>
	...
	/**
	 * Registers property type binders by extracting the key and value from each
	 * entry in the provided map using the key to specify the property type and the
	 * value to specify the binder.
	 * 
	 * <p>
	 * Binders specified in the provided map will override any binders previously
	 * registered for the same property type.
	 *
	 * @param binders the map containing the entries to register; keys must be of
	 *                type <code>Class</code> and values of type
	 *                <code>Binder</code>.
	 */
	...
	/**
	 * Registers control type binders by extracting the key and value from each
	 * entry in the provided map using the key to specify the property type and the
	 * value to specify the binder.
	 * 
	 * <p>
