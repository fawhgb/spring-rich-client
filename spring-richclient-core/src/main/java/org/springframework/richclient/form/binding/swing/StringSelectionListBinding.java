...
public class StringSelectionListBinding extends CustomBinding {
	 ...
	 * Constructor.
	 * 
	 * @param comboBox         JComboBox to use with this binding.
	 * @param formModel        formModel containing the property.
	 * @param formPropertyPath path to property.
	 * @param readOnly         force readOnly.
	 */
	...
	/**
	 * Will resolve keys to messages using 'id'.'key'.label as a message resource
	 * key.
	 * 
	 * @param id   id to use with message key resolving.
	 * @param keys keys to extract messages from.
	 * @return array containing labels/messages for each key.
	 */
	private static String[] createLabels(final String id, final Object[] keys) {
		String[] messages = new String[keys.length];
		for (int i = 0; i < messages.length; ++i) {
			messages[i] = RcpSupport.getMessage(id, keys[i] == null ? "null" : keys[i].toString(), RcpSupport.LABEL);
		}
		return messages;
	}
	 ...
	 * 
	 * @param sortedKeys    collection with keys in the appropriate order. Each key
	 *                      corresponds to one in the map.
	 * @param keysAndLabels map containing both, keys and labels, from which the
	 *                      labels have to be extracted.
	 * @return a collection containing the labels corresponding to the collection of
	 *         sortedKeys.
	 */
	 ...
	 * When the item doesn't map to an entry in the comboBoxModel, select a default
	 * one. See {@link #getDefaultKeyIndex()} and {@link #setDefaultKeyIndex(int)}
	 * for default.
	 * 
	 * @param item object that needs to be selected in the comboBox.
	 */
	...
	/**
	 * Arrays.binarySearch(..) does exist in java, but assumes that array is sorted,
	 * we don't have this restriction so dumb search from first to last...
	 * 
	 * @param keys
	 * @param toFind
	 * @return
	 ...
	 * comboBox list.
	 * 
	 * @param defaultKeyIndex default index to select.
	 */
	...
	/**
	 * PreSelect hook. Define additional things to be executed before selecting the
	 * propertyValue in the comboBox list.
	 */
	...
	/**
	 * @param keys array of keys to set, use keys as labels.
	 ...
	 * 
	 * @param keys   array of objects to use as keys.
	 * @param labels array of objects to use as labels in the comboBox.
	 */
	...
	/**
	 * @param keys collection of keys to set, use keys as labels.
	 * @see #setSelectionList(Object[], Object[])
	 */
	...
	/**
	 * @param keys   collection of keys to use, corresponding to the labels.
	 * @param labels collection of labels to use, corresponding to the keys.
	 * @see #setSelectionList(Object[], Object[])
	 */
	...
	/**
	 * @param sortedKeys    sorted subcollection of map containing the needed keys.
	 * @param keysAndLabels map containing the keys (supercollection) and mapped
	 *                      labels.
	 * @see #setSelectionList(Object[], Object[])
	 */
	...
	/**
	 * @param keysAndLabels map containing keys and labels to set in comboBox.
	 * @see #setSelectionList(Object[], Object[])
	 */
	 ...
	 * 
	 * @param id to use for message keys.
	 */
