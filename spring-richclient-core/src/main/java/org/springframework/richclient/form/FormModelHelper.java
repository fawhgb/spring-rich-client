...
/**
 * This provides a collection of useful functions for working with
 * {@link FormModel}s.
 *
 * @author Keith Donald
 * @author Jim Moore
...
public class FormModelHelper {
	 ...
	 * provided name. The form object associated with the created child model is the
	 * value model at the specified parent property path.
	 *
	 * @param parentModel                 the model to create the FormModelHelper in
	 * @param childPageName               the name to associate the created
	 *                                    FormModelHelper with in the groupingModel
	 * @param childFormObjectPropertyPath the path into the groupingModel that the
	 *                                    FormModelHelper is for
	 *
	...
	public static FormModel getChild(HierarchicalFormModel formModel, String childPageName) {
		...
		for (int i = 0; i < children.length; i++) {
			...
			if (childPageName.equals(child.getId())) {
				return child;
			}
		}
