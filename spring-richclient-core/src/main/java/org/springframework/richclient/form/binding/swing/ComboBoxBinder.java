...
public class ComboBoxBinder extends AbstractListBinder {
	...
	/**
	 * context key for a value which is used to mark an empty Selection. If this
	 * value is selected null will be assigned to the fields value
	 */
	...
	protected void applyContext(AbstractListBinding binding, Map context) {
		...
		if (context.containsKey(RENDERER_KEY)) {
			comboBoxBinding
					.setRenderer((ListCellRenderer) decorate(context.get(RENDERER_KEY), comboBoxBinding.getRenderer()));
	...
	/**
	 * Defines a closure which is called to create the renderer. The argument for
	 * the closure will be the default renderer (see {@link JComboBox#getRenderer()}
	 * of the combobox. The closure must create an instance of
	 * {@link ListCellRenderer}
	 * 
	 * @param rendererClosure the closure which is used to create the renderer
	 */
	...
	/**
	 * Defines a closure which is called to create the editor. The argument for the
	 * closure will be the default editor (see {@link JComboBox#getEditor()} of the
	 * combobox. The closure must create an instance of {@link ComboBoxEditor}
	 * 
	 * @param editorClosure the closure which is used to create the editor
	 */
