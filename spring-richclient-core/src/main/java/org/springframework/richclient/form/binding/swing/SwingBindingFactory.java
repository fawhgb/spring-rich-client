...
/**
 * A convenient implementation of <code>BindingFactory</code>. Provides a set of
 * methods that address the typical binding requirements of Swing based forms.
 *
...
public class SwingBindingFactory extends AbstractBindingFactory {
	...
	public Binding createBoundComboBox(String formProperty, Object selectableItems) {
		Map context = createContext(AbstractListBinder.SELECTABLE_ITEMS_KEY, selectableItems);
	...
	public ObservableList createBoundListModel(String formProperty) {
		...
		if (!(valueModel instanceof BufferedCollectionValueModel)) {
			...
			valueModel = new BufferedCollectionValueModel(
					(((DefaultFormModel) formModel).getFormObjectPropertyAccessStrategy()).getPropertyValueModel(
							formProperty),
					formModel.getFieldMetadata(formProperty).getPropertyType());
	...
	public Binding createBoundList(String formProperty) {
		Map context = createContext(AbstractListBinder.SELECTABLE_ITEMS_KEY, createBoundListModel(formProperty));
		return createBinding(JList.class, formProperty, context);
	}

	/**
	 * Binds the values specified in the collection contained within
	 * <code>selectableItems</code> to a {@link JList}, with any user selection
	 * being placed in the form property referred to by
	 * <code>selectionFormProperty</code>. Each item in the list will be rendered as
	 * a String. Note that the selection in the bound list will track any changes to
	 * the <code>selectionFormProperty</code>. This is especially useful to
	 * preselect items in the list - if <code>selectionFormProperty</code> is not
	 * empty when the list is bound, then its content will be used for the initial
	 * selection. This method uses default behavior to determine the selection mode
	 * of the resulting <code>JList</code>: if <code>selectionFormProperty</code>
	 * refers to a {@link java.util.Collection} type property, then
	 * {@link javax.swing.ListSelectionModel#MULTIPLE_INTERVAL_SELECTION} will be
	 * used, otherwise {@link javax.swing.ListSelectionModel#SINGLE_SELECTION} will
	 * be used.
	 * 
	 * @param selectionFormProperty form property to hold user's selection. This
	 *                              property must either be compatible with the item
	 *                              objects contained in
	 *                              <code>selectableItemsHolder</code> (in which
	 *                              case only single selection makes sense), or must
	 *                              be a <code>Collection</code> type, which allows
	 *                              for multiple selection.
	 * @param selectableItems       a Collection or array containing the items with
	 *                              which to populate the list.
	 * @return
	 */
	...
	/**
	 * Binds the values specified in the collection contained within
	 * <code>selectableItemsHolder</code> to a {@link JList}, with any user
	 * selection being placed in the form property referred to by
	 * <code>selectionFormProperty</code>. Each item in the list will be rendered by
	 * looking up a property on the item by the name contained in
	 * <code>renderedProperty</code>, retrieving the value of the property, and
	 * rendering that value in the UI. Note that the selection in the bound list
	 * will track any changes to the <code>selectionFormProperty</code>. This is
	 * especially useful to preselect items in the list - if
	 * <code>selectionFormProperty</code> is not empty when the list is bound, then
	 * its content will be used for the initial selection. This method uses default
	 * behavior to determine the selection mode of the resulting <code>JList</code>:
	 ...
	 *                              be a <code>Collection</code> type, which allows
	 *                              for multiple selection.
	 * @param selectableItemsHolder <code>ValueModel</code> containing the items
	 *                              with which to populate the list.
	 * @param renderedProperty      the property to be queried for each item in the
	 *                              list, the result of which will be used to render
	 *                              that item in the UI
	 * 
	 * @return
	 */
	...
	/**
	 * Binds the value(s) specified in <code>selectableItems</code> to a
	 * {@link JList}, with any user selection being placed in the form property
	 * referred to by <code>selectionFormProperty</code>. Each item in the list will
	 * be rendered by looking up a property on the item by the name contained in
	 * <code>renderedProperty</code>, retrieving the value of the property, and
	 * rendering that value in the UI. Note that the selection in the bound list
	 * will track any changes to the <code>selectionFormProperty</code>. This is
	 * especially useful to preselect items in the list - if
	 * <code>selectionFormProperty</code> is not empty when the list is bound, then
	 * its content will be used for the initial selection.
	 * 
	 * @param selectionFormProperty form property to hold user's selection. This
	 *                              property must either be compatible with the item
	 *                              objects contained in
	 ...
	 *                              be a <code>Collection</code> type, which allows
	 *                              for multiple selection.
	 * @param selectableItems       <code>Object</code> containing the item(s) with
	 *                              which to populate the list. Can be an instance
	 *                              Collection, Object[], a ValueModel or Object
	 * @param renderedProperty      the property to be queried for each item in the
	 *                              list, the result of which will be used to render
	 *                              that item in the UI. May be null, in which case
	 *                              the selectable items will be rendered as
	 *                              strings.
	 * @param forceSelectMode       forces the list selection mode. Must be one of
	 *                              the constants defined in
	 *                              {@link javax.swing.ListSelectionModel} or
	 *                              <code>null</code> for default behavior. If
	 *                              <code>null</code>, then
	 *                              {@link javax.swing.ListSelectionModel#MULTIPLE_INTERVAL_SELECTION}
	 *                              will be used if
	 *                              <code>selectionFormProperty</code> refers to a
	 *                              {@link java.util.Collection} type property,
	 *                              otherwise
	...
	public Binding createBoundList(String selectionFormProperty, Object selectableItems, String renderedProperty,
		...
		context.put(AbstractListBinder.SELECTABLE_ITEMS_KEY, selectableItems);
	...
	/**
	 * Binds the values specified in the collection contained within
	 * <code>selectableItemsHolder</code> to a
	 * {@link org.springframework.richclient.components.ShuttleList}, with any user
	 * selection being placed in the form property referred to by
	 * <code>selectionFormProperty</code>. Each item in the list will be rendered by
	 * looking up a property on the item by the name contained in
	 * <code>renderedProperty</code>, retrieving the value of the property, and
	 * rendering that value in the UI.
	 * <p>
	 * Note that the selection in the bound list will track any changes to the
	 * <code>selectionFormProperty</code>. This is especially useful to preselect
	 * items in the list - if <code>selectionFormProperty</code> is not empty when
	 * the list is bound, then its content will be used for the initial selection.
	 *
	 * @param selectionFormProperty form property to hold user's selection. This
	 *                              property must be a <code>Collection</code> or
	 *                              array type.
	 * @param selectableItemsHolder <code>ValueModel</code> containing the items
	 *                              with which to populate the list.
	 * @param renderedProperty      the property to be queried for each item in the
	 ...
	 *                              the selectable items will be rendered as
	 *                              strings.
	 * @return constructed {@link Binding}. Note that the bound control is of type
	 *         {@link org.springframework.richclient.components.ShuttleList}. Access
	 *         this component to set specific display properties.
	 */
	...
	/**
	 * Binds the values specified in the collection contained within
	 * <code>selectableItems</code> (which will be wrapped in a {@link ValueHolder}
	 * to a {@link ShuttleList}, with any user selection being placed in the form
	 * property referred to by <code>selectionFormProperty</code>. Each item in the
	 * list will be rendered by looking up a property on the item by the name
	 * contained in <code>renderedProperty</code>, retrieving the value of the
	 ...
	 *                              which to populate the selectable list (this
	 *                              object will be wrapped in a ValueHolder).
	 * @param renderedProperty      the property to be queried for each item in the
	 *                              list, the result of which will be used to render
	 *                              that item in the UI. May be null, in which case
	 *                              the selectable items will be rendered as
	 *                              strings.
	 * @return constructed {@link Binding}. Note that the bound control is of type
	 *         {@link ShuttleList}. Access this component to set specific display
	 *         properties.
	 */
	 ...
	 * to a {@link ShuttleList}, with any user selection being placed in the form
	 * property referred to by <code>selectionFormProperty</code>. Each item in the
	 * list will be rendered as a String.
	 * <p>
	 * Note that the selection in the bound list will track any changes to the
	 * <code>selectionFormProperty</code>. This is especially useful to preselect
	 * items in the list - if <code>selectionFormProperty</code> is not empty when
