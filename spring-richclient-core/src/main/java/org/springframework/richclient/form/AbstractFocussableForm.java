...
public abstract class AbstractFocussableForm extends AbstractWidgetForm implements Focussable// , SecurityControllable
{
	...
	/**
	 * Override to do nothing. Superclass registers a default command, but we are
	 * using a different system to define default commands.
	 */
	@Override
	...
	public boolean canClose() {
		...
		if (this.getFormModel().isEnabled() && this.getFormModel().isDirty()
				&& this.getCommitCommand().isAuthorized()) { // then we ask the user to save the mess first:
																// yes/no/cancel
