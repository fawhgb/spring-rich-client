...
/**
 * Wizard dialog that additionally adds a panel to the dialog showing the page
 * path and the current page.
 *
 * @author Schaubroeck N.V.
 */
public class ExtendedWizardDialog extends org.springframework.richclient.wizard.WizardDialog {
	...
	public ExtendedWizardDialog(Wizard wizard, String id) {
		...
		this.id = id;
		if (this.id != null) {
			((ApplicationObjectConfigurer) ApplicationServicesLocator.services()
					.getService(ApplicationObjectConfigurer.class)).configure(this, this.id);
		}
	}
	...
	protected JComponent createDialogContentPane() {
		JPanel wizardPanel = new JPanel(new FormLayout(
				new ColumnSpec[] { new ColumnSpec(ColumnSpec.FILL, Sizes.DEFAULT, FormSpec.NO_GROW),
		...
		wizardPanel.add(createWizardIndex(), cc.xy(1, 1)); // do this after super.createDialogPane() because only then,
															// pages are added
	...
	private Component createWizardIndex() {
		JPanel indexPanel = new JPanel(
				new FormLayout(new ColumnSpec[] { new ColumnSpec(ColumnSpec.CENTER, Sizes.DEFAULT, FormSpec.NO_GROW) },
