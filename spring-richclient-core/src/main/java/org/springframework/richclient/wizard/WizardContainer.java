 ...
 * returns the current page; <code>showPage</code> programmatically changes the
 * the current page. Note that the pages need not all belong to the same wizard.
 * <p>
...
public interface WizardContainer {
	 ...
	 * 
	 * @return the current wizard page, or <code>null</code> if the container is not
	 *         yet showing the wizard
	 * @see #showPage
	 */
	 ...
	 * <p>
	 * This method should not be use for normal page sequencing (back, next) which
	 * is handled by the container itself. It may, however, be used to move to
	 * another page in response to some custom action such as double clicking in a
	 * list.
	 * </p>
	 * 
