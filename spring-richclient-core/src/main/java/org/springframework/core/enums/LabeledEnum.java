...
/**
 * An interface for objects that represent a labeled enumeration. Each such enum
 * instance has the following characteristics:
 *
 * <ul>
 * <li>A type that identifies the enum's class. For example:
 * <code>com.mycompany.util.FileFormat</code>.</li>
 *
 * <li>A code that uniquely identifies the enum within the context of its type.
 * For example: &quot;CSV&quot;. Different classes of codes are possible (e.g.,
 * Character, Integer, String).</li>
 *
...
public interface LabeledEnum extends Comparable, Serializable {
	...
	/**
	 * Shared Comparator instance that sorts enumerations by
	 * <code>LABEL_ORDER</code>, then <code>CODE_ORDER</code>.
	 */
	Comparator DEFAULT_ORDER = new CompoundComparator(new Comparator[] { LABEL_ORDER, CODE_ORDER });
