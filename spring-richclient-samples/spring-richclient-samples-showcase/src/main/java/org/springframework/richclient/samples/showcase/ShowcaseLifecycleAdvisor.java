 ...
 * the License.
 */
package org.springframework.richclient.samples.showcase;
 ...
 * @author Keith Donald
 */
public class ShowcaseLifecycleAdvisor extends DefaultApplicationLifecycleAdvisor {

	private final Log logger = LogFactory.getLog(getClass());

	/**
	 * This method is called prior to the opening of an application window. Note at
	 * this point the window control has not been created. This hook allows
	 * programmatic control over the configuration of the window (by setting
	 * properties on the configurer) and it provides a hook where code that needs to
	 * be executed prior to the window opening can be plugged in (like a startup
	 * wizard, for example).
	 * 
	...
	/**
	 * Called just after the command context has been internalized. At this point,
	 * all the commands for the window have been created and are available for use.
	 * If you need to force the execution of a command prior to the display of an
	 * application window (like a login command), this is where you'd do it.
	 * 
	...
	/**
	 * Called when the window is being closed. This hook allows control over whether
	 * the window is allowed to close. By returning false from this method, the
	 * window will not be closed.
	 * 
	 * @return boolean indicator if window should be closed. <code>true</code> to
	 *         allow the close, <code>false</code> to prevent the close.
	 */
	@Override
