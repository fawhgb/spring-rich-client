...
public class ContactForm extends AbstractFocussableForm {
	...
	protected JComponent createFormControl() {
		FormLayout layout = new FormLayout("right:pref, 4dlu, fill:pref:grow, 6dlu, right:pref, 4dlu, fill:pref:grow",
				"default");
		...
		formBuilder.nextRow();
		// formBuilder.add(getBindingFactory().createBoundComboBox( "address.state",
		 ...
		 * 
		 * formBuilder.addSeparator("General"); formBuilder.row(); firstNameField =
		 * formBuilder.add("firstName")[1]; formBuilder.add("lastName");
		 * formBuilder.row(); formBuilder.add("dateOfBirth", "colSpan=1");
		 * formBuilder.row(); formBuilder.add("homePhone");
		 * formBuilder.add("workPhone"); formBuilder.row();
		 * formBuilder.add("emailAddress"); formBuilder.row(); formBuilder.row();
		 * formBuilder.add("contactType", "colSpan=1 align=left"); formBuilder.row();
		 * formBuilder.addSeparator("Address"); formBuilder.row();
		 * formBuilder.add("address.address1"); formBuilder.row();
		 * formBuilder.add("address.address2"); formBuilder.row();
		 * formBuilder.add("address.address3"); formBuilder.row();
		 * formBuilder.add("address.city", "colSpan=1 align=left"); formBuilder.row();
		 * // formBuilder.add(getBindingFactory().createBoundComboBox( "address.state",
		 * MasterLists.STATE_CODE), "colSpan=1 align=left" );
		 * formBuilder.add("address.state", "colSpan=1 align=left"); formBuilder.row();
		 * 
		 * // We want to make the zip code UI field smaller than the default. The add
		 * method // returns an array of two components, the field label and the
		 * component bound to // the property. JComponent zipField =
		 * formBuilder.add("address.zip", "colSpan=1 align=left")[1]; ((JTextField)
		 * zipField).setColumns(8); formBuilder.row();
		 */
