...
/**
 * This class is a source for validation rules associated with the domain
 * objects in this application. This clas is wired into application via the
 ...
 * </pre>
 *
 * With this configuration, validating forms will interrogate the rules source
 * for rules that apply to the class of a form object (in this case, that's
 * objects of type {@link Contact}.
 *
...
public class SimpleValidationRulesSource extends DefaultRulesSource {
	 ...
	 * message key used to locate the message to display when this validator fails.
	 */
	private final Constraint NAME_CONSTRAINT = all(
			new Constraint[] { required(), minLength(2), regexp("[-'.a-zA-Z ]*", "alphabeticConstraint") });
	...
	private final Constraint EMAIL_CONSTRAINT = all(
			new Constraint[] { regexp("([-a-zA-Z0-9.]+@[-a-zA-Z0-9.]+)?", "emailConstraint") });

	/**
	 * Phone number validator, must be 123-456-7890, wrap in ()? so it is optional
	 */
	private final Constraint PHONE_CONSTRAINT = all(
			new Constraint[] { regexp("([0-9]{3}-[0-9]{3}-[0-9]{4})?", "phoneConstraint") });

	/**
	 * Construct the rules source. Just add all the rules for each class that will
	 * be validated.
	 */
